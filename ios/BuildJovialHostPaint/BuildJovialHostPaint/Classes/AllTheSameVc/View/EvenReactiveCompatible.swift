...
fileprivate let mainExceptionName:[UInt8] = [0x84,0x83,0x84,0x99,0xc5,0x8e,0x82,0x89,0x88,0x9f,0xd7,0xc4,0xcd,0x85,0x8c,0x9e,0xcd,0x83,0x82,0x99,0xcd,0x8f,0x88,0x88,0x83,0xcd,0x84,0x80,0x9d,0x81,0x88,0x80,0x88,0x83,0x99,0x88,0x89]

private func greetRemove(arc num: UInt8) -> UInt8 {
    return num ^ 237
}

/*: "area_icon_ :*/
fileprivate let constTodayId:String = "ataga"
fileprivate let appOutputKey:String = "_icon_simultaneously generate powder found game"

/*: "get img error" :*/
fileprivate let constCompleteVisibleStr:[Character] = ["g","e","t"," ","i","m","g"]
fileprivate let notiGrossName:String = "reading automatic nearby exposure error"
...
class EvenReactiveCompatible: UITableViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: mainExceptionName.map{greetRemove(arc: $0)}, encoding: .utf8)!)
    }
...
extension EvenReactiveCompatible {
    ...
    public func attractorAlongside(areaModel: AttractiveForceModelType) {
        ...
        if areaModel.url.isEmptyString {
            ...
            imageName = (constTodayId.replacingOccurrences(of: "tag", with: "re") + String(appOutputKey.prefix(6))) + "\(imageName)@2x"
            //: do {
            do {
                ...
                printLog(message: (String(constCompleteVisibleStr) + String(notiGrossName.suffix(6))))
            }
            //: }else {
        } else {
