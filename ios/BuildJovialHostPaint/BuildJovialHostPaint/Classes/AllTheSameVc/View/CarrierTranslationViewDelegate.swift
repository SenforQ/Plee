...
fileprivate let data_consequenceFormat:[UInt8] = [0xe0,0xe7,0xe0,0xfd,0xa1,0xea,0xe6,0xed,0xec,0xfb,0xb3,0xa0,0xa9,0xe1,0xe8,0xfa,0xa9,0xe7,0xe6,0xfd,0xa9,0xeb,0xec,0xec,0xe7,0xa9,0xe0,0xe4,0xf9,0xe5,0xec,0xe4,0xec,0xe7,0xfd,0xec,0xed]

private func nearTransfer(add num: UInt8) -> UInt8 {
    return num ^ 137
}

/*: "Nickname" :*/
fileprivate let main_cancelMessage:[Character] = ["N","i","c","k","n","a","m","e"]

/*: "Birthday" :*/
fileprivate let dataPauseTitle:String = "task gross menu step tarBi"
fileprivate let notiAgoSuccessThanksName:String = "RTHDAY"

/*: "F4F4F4" :*/
fileprivate let const_donFormat:[Character] = ["F","4","F","4","F","4"]

/*: "area_code_down_icon" :*/
fileprivate let userGlassCornerName:String = "arspeaka"
fileprivate let show_surroundName:String = "_down_icobad population"
fileprivate let noti_shouldPath:String = "gravity"

/*: "BBBBBB" :*/
fileprivate let kMouthFormat:[Character] = ["B","B","B","B","B","B"]

/*: "0/20" :*/
fileprivate let appCookieData:[Character] = ["0","/","2","0"]

/*: "20/0" :*/
fileprivate let app_reasonMsg:String = "2scale/scale"

/*: "Invitation code" :*/
fileprivate let main_alreadyMsg:[Character] = ["I","n","v","i","t","a"]
fileprivate let kFlagName:[Character] = ["t","i","o","n"," ","c","o","d","e"]

/*: "btn_field_delete_icon" :*/
fileprivate let mainNaturalSimplyName:String = "false ownbtn_fi"
fileprivate let mainGiftRefreshlyData:String = "lpolicytpolicy"
fileprivate let mainOffdId:String = "food just recording_icon"

/*: "Next" :*/
fileprivate let mainLocalChoosePath:String = "guide threadNext"

/*: "#8C7AFF" :*/
fileprivate let mainConversationPath:String = "aim computer stop#8C7AFF"

/*: "Skip" :*/
fileprivate let show_dialogData:String = "cancel football librarySkip"
...
// __CLOSE_PRINT__
//
//  CarrierTranslationViewDelegate.swift
//  AbroadTalking
//
...
class CarrierTranslationViewDelegate: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.white
        //: RealmReactiveCompatible.share.userFillInfoMode.nickName = RealmReactiveCompatible.share.loginUserMode.nickname ?? ""
        RealmReactiveCompatible.share.userFillInfoMode.nickName = RealmReactiveCompatible.share.loginUserMode.nickname ?? ""
        //: if let inviteCode = UserDefaults.standard.string(forKey: SaveInviteCodeKey) {
        if let inviteCode = UserDefaults.standard.string(forKey: constFeatureId) {
            //: RealmReactiveCompatible.share.userFillInfoMode.inviteCode = inviteCode
            RealmReactiveCompatible.share.userFillInfoMode.inviteCode = inviteCode
        ...
        //: if (RealmReactiveCompatible.share.userFillInfoMode.sex == Gender.female.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeFemale) ||
        if (RealmReactiveCompatible.share.userFillInfoMode.sex == FullHashable.female.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeFemale) ||
            //: (RealmReactiveCompatible.share.userFillInfoMode.sex == Gender.male.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeMale) {
            (RealmReactiveCompatible.share.userFillInfoMode.sex == FullHashable.male.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeMale)
        {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_consequenceFormat.map{nearTransfer(add: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameTitleLab: UILabel = {
        ...
        label.text = (String(main_cancelMessage)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var birthTitleLab: UILabel = {
        ...
        label.text = (String(dataPauseTitle.suffix(2)) + notiAgoSuccessThanksName.lowercased()).localized
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var birthControl: UIControl = {
        ...
        control.backgroundColor = UIColor(hex: (String(const_donFormat)))
    ...
    lazy var birthLabel: UILabel = {
        ...
        //: label.textColor = .appTitleColor()
        label.textColor = .askNameColor()
        //: let day = RealmReactiveCompatible.share.userFillInfoMode.birthDay
        let day = RealmReactiveCompatible.share.userFillInfoMode.birthDay
        //: let month = RealmReactiveCompatible.share.userFillInfoMode.birthMonth
        let month = RealmReactiveCompatible.share.userFillInfoMode.birthMonth
        //: let year = RealmReactiveCompatible.share.userFillInfoMode.birthYear
        let year = RealmReactiveCompatible.share.userFillInfoMode.birthYear
    ...
    lazy var birthDownImgView: UIImageView = {
        ...
        imageView.image = UIImage.adjust(name: (userGlassCornerName.replacingOccurrences(of: "speak", with: "e") + "_code" + String(show_surroundName.prefix(9)) + noti_shouldPath.replacingOccurrences(of: "gravity", with: "n")))
        //: return imageView
        return imageView
        //: }()
    }()
    ...
    lazy var nameInputView: UITextField = {
        ...
        textField.backgroundColor = UIColor(hex: (String(const_donFormat)))
        ...
        textField.attributedPlaceholder = NSAttributedString(string: "", attributes: [NSAttributedString.Key.font: UIFont.shrinkSize(type: .Regular, fontSize: 15), NSAttributedString.Key.foregroundColor: UIColor(hex: (String(kMouthFormat)))!])
        ...
        textField.addTarget(self, action: #selector(app(_:)), for: .editingChanged)
        //: textField.text = RealmReactiveCompatible.share.userFillInfoMode.nickName
        textField.text = RealmReactiveCompatible.share.userFillInfoMode.nickName
        //: return textField
        return textField
        //: }()
    ...
    lazy var nameCountLab: UILabel = {
        ...
        label.textAlignment = .right
        //: label.text = "0/20"
        label.text = (String(appCookieData))
        //: if LanguageManager.shared.direction == .rightToLeft {
        if RepresentLanguageManager.shared.direction == .rightToLeft {
            //: label.text = "20/0"
            label.text = (app_reasonMsg.replacingOccurrences(of: "scale", with: "0"))
        }
        //: return label
        return label
        //: }()
    ...
    private lazy var inviteCodeTitleLab: UILabel = {
        ...
        label.text = (String(main_alreadyMsg) + String(kFlagName)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var inviteCodeCleanBtn: UIButton = {
        ...
        btn.setImage(UIImage.adjust(name: (String(mainNaturalSimplyName.suffix(6)) + "eld_de" + mainGiftRefreshlyData.replacingOccurrences(of: "policy", with: "e") + String(mainOffdId.suffix(5)))), for: .normal)
    ...
    lazy var inviteCodeInputView: UITextField = {
        ...
        textField.addTarget(self, action: #selector(fragment(_:)), for: .editingChanged)
        //: return textField
        return textField
        //: }()
    }()
    ...
    lazy var commitButton: UIButton = {
        ...
        btn.setTitle((String(mainLocalChoosePath.suffix(4))).localized, for: .normal)
    ...
    lazy var skipButton: UIButton = {
        ...
        btn.setTitleColor(UIColor(hex: (String(mainConversationPath.suffix(7)))), for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 17)
        btn.titleLabel?.font = UIFont.shrinkSize(type: .Medium, fontSize: 17)
        //: let attributeString = NSMutableAttributedString(string: "Skip".localized)
        let attributeString = NSMutableAttributedString(string: (String(show_dialogData.suffix(4))).localized)
...
extension CarrierTranslationViewDelegate {
    /// 更新Next按钮状态
    //: private func func__checkFinishBtnState() {
    private func seeThroughState() {
        //: if (RealmReactiveCompatible.share.userFillInfoMode.sex == Gender.female.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeFemale) ||
        if (RealmReactiveCompatible.share.userFillInfoMode.sex == FullHashable.female.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeFemale) ||
            //: (RealmReactiveCompatible.share.userFillInfoMode.sex == Gender.male.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeMale) {
            (RealmReactiveCompatible.share.userFillInfoMode.sex == FullHashable.male.rawValue && !RealmReactiveCompatible.share.appConfigMode.skipInputInviteCodeMale)
        { // 全部必填
            //: let nickName = RealmReactiveCompatible.share.userFillInfoMode.nickName
            let nickName = RealmReactiveCompatible.share.userFillInfoMode.nickName
            //: let inviteCode = RealmReactiveCompatible.share.userFillInfoMode.inviteCode
            let inviteCode = RealmReactiveCompatible.share.userFillInfoMode.inviteCode
...
extension CarrierTranslationViewDelegate: UITextFieldDelegate {
    //: @objc private func nicknameTextChage(_ textField: UITextField) {
    @objc private func app(_: UITextField) {
        ...
        //: RealmReactiveCompatible.share.userFillInfoMode.nickName = nameInputView.text ?? ""
        RealmReactiveCompatible.share.userFillInfoMode.nickName = nameInputView.text ?? ""
        //: func__checkFinishBtnState()
        seeThroughState()
    }
    ...
    @objc private func fragment(_: UITextField) {
        ...
        //: RealmReactiveCompatible.share.userFillInfoMode.inviteCode = inviteCodeInputView.text ?? ""
        RealmReactiveCompatible.share.userFillInfoMode.inviteCode = inviteCodeInputView.text ?? ""
        //: func__checkFinishBtnState()
        seeThroughState()
    }
...
extension CarrierTranslationViewDelegate: InconsistentViewDelegate {
    ...
    func checkreinAction() {
        ...
        let year = RealmReactiveCompatible.share.userFillInfoMode.birthYear
        //: myDateView.showView()
        myDateView.concentration()
        //: DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            //: myDateView.setDefaultDate(year: year, month: month, day: day)
            myDateView.verticalDay(year: year, month: month, day: day)
        }
    }

    //: func pickDateView(year: Int, month: Int, day: Int) {
    func orientation(year: Int, month: Int, day: Int) {
        //: birthLabel.text = "\(String(format: "%.2d", month))-\(String(format: "%.2d", day))-\(year)"
        birthLabel.text = "\(String(format: "%.2d", month))-\(String(format: "%.2d", day))-\(year)"
        //: RealmReactiveCompatible.share.userFillInfoMode.birthDay = day
        RealmReactiveCompatible.share.userFillInfoMode.birthDay = day
        //: RealmReactiveCompatible.share.userFillInfoMode.birthMonth = month
        RealmReactiveCompatible.share.userFillInfoMode.birthMonth = month
        //: RealmReactiveCompatible.share.userFillInfoMode.birthYear = year
        RealmReactiveCompatible.share.userFillInfoMode.birthYear = year
    }
}
