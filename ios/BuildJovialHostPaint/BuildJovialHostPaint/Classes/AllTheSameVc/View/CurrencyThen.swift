...
fileprivate let user_leaveName:[UInt8] = [0x95,0x92,0x95,0x88,0xd4,0x9f,0x93,0x98,0x99,0x8e,0xc6,0xd5,0xdc,0x94,0x9d,0x8f,0xdc,0x92,0x93,0x88,0xdc,0x9e,0x99,0x99,0x92,0xdc,0x95,0x91,0x8c,0x90,0x99,0x91,0x99,0x92,0x88,0x99,0x98]

private func usSub(statement num: UInt8) -> UInt8 {
    return num ^ 252
}

/*: "8075F5" :*/
fileprivate let mainBirthName:String = "8075F5bean break clean island presentation"

/*: "9A9B9C" :*/
fileprivate let main_keepKey:[Character] = ["9","A","9","B","9","C"]

/*: "kOpacityAnimation" :*/
fileprivate let k_nothingPickVeryFormat:[Character] = ["k","O","p","a","c","i","t","y","A","n","i","m","a"]
fileprivate let show_subjectPath:String = "tioyield"

/*: "opacity" :*/
fileprivate let app_indicatorMessage:[Character] = ["o","p","a","c","i","t","y"]

/*: "forwards" :*/
fileprivate let appPrettyData:String = "forpause"
fileprivate let app_watchName:String = "rdfore"

/*: "easeIn" :*/
fileprivate let dataBecomeQuoteMessage:String = "easeIncur reject each ok security"

/*: "8C7AF8" :*/
fileprivate let app_gapTitle:[Character] = ["8","C","7","A","F","8"]
...
class CurrencyThen: UIView {
    ...
    public init(frame: CGRect, codeNumber: Int = 6, labelTextColor: UIColor = UIColor.black, mainColor: UIColor = UIColor.orange, normalColor: UIColor = UIColor.gray, margin: CGFloat = 12.0) {
        ...
        self.focusBeautyConstraint()
        //: self.bindInteraction()
        self.holdFastWithoutContact()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: user_leaveName.map{usSub(statement: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(solarDay(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension CurrencyThen {
    ...
    func toolBy() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(mainBirthName.prefix(6))))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(main_keepKey)))!.withAlphaComponent(0.25).cgColor
    ...
    fileprivate func begin(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(k_nothingPickVeryFormat) + show_subjectPath.replacingOccurrences(of: "yield", with: "n")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(downAnimation(), forKey: (String(k_nothingPickVeryFormat) + show_subjectPath.replacingOccurrences(of: "yield", with: "n")))
        }
    ...
    fileprivate func downAnimation() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(app_indicatorMessage)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (appPrettyData.replacingOccurrences(of: "pause", with: "wa") + app_watchName.replacingOccurrences(of: "fore", with: "s")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(dataBecomeQuoteMessage.prefix(6)))))
        //: return animation
        return animation
    }
}
...
extension CurrencyThen {
    ...
    private func streetSmart() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (String(app_gapTitle)))?.cgColor
    ...
    private func focusBeautyConstraint() {}

    // 添加事件
    //: private func bindInteraction() {
    private func holdFastWithoutContact() {}
}
