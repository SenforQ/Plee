...
fileprivate let app_transformationFormat:[UInt8] = [0x44,0x49,0x44,0x4f,0x3,0x3e,0x4a,0x3f,0x40,0x4d,0x15,0x4,0xfb,0x43,0x3c,0x4e,0xfb,0x49,0x4a,0x4f,0xfb,0x3d,0x40,0x40,0x49,0xfb,0x44,0x48,0x4b,0x47,0x40,0x48,0x40,0x49,0x4f,0x40,0x3f]

fileprivate func dialogPanel(graphic num: UInt8) -> UInt8 {
...
/*: "img_home_shadow" :*/
fileprivate let notiPopGumKey:[Character] = ["i","m","g","_","h","o","m","e","_","s"]
fileprivate let dataNoticeId:String = "placeadow"

/*: "#9610FF" :*/
fileprivate let mainOperativeData:[Character] = ["#","9"]
fileprivate let main_commentId:String = "yourself immune required rein610FF"

/*: "#8566FF" :*/
fileprivate let data_spaceSlowName:String = "family position population#8566FF"

/*: "Random Video" :*/
fileprivate let mainSeekThreeMessage:[Character] = ["R","a","n","d","o","m"," ","V","i","d","e","o"]

/*: "icon_match_call_free" :*/
fileprivate let k_customMultiLawText:String = "icon_connection function limit"
fileprivate let const_himName:String = "cover recognize catch material keyll_free"

/*: "Free" :*/
fileprivate let user_extraFormat:[Character] = ["F","r","e","e"]

/*: "Scroll_upto_discove" :*/
fileprivate let kGreenScenePath:[Character] = ["S","c","r","o","l","l","_","u","p","t","o","_","d","i","s","c","o","v","e"]

/*: "icon_VectorStroke" :*/
fileprivate let mainTrackId:[Character] = ["i","c","o","n","_","V","e","c","t","o","r","S","t","r","o","k","e"]

/*: "get json error" :*/
fileprivate let appHugeData:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r","o"]
fileprivate let showHearingKey:[Character] = ["r"]
...
class OutsideView: UIView {
    ...
    var maxRemoveDistance = dataModeBlockStr / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [MultiExtendView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: LostObjectProtocol? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: TranslationViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_transformationFormat.map{dialogPanel(graphic: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.adjust(name: (String(notiPopGumKey) + dataNoticeId.replacingOccurrences(of: "place", with: "h")))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.thumbNumbererest(colors: [UIColor(hex: (String(mainOperativeData) + String(main_commentId.suffix(5))))!.cgColor, UIColor(hex: (String(data_spaceSlowName.suffix(7))))!.cgColor], size: CGSize(width: dataModeBlockStr - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(mainSeekThreeMessage)).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(clewStack), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.adjust(name: (String(k_customMultiLawText.prefix(5)) + "match_ca" + String(const_himName.suffix(7)))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(user_extraFormat)).localized, for: .normal)
...
extension OutsideView {
    ...
    private func asDrown() {
        ...
        image.image = UIImage.adjust(name: (String(kGreenScenePath)))
        ...
        image2.image = UIImage.adjust(name: (String(mainTrackId)))
        ...
        estimatedRevenue(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func estimatedRevenue(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = PortraitJackEffectTool.default.responsibility(type: .Club_guidePage)
    ...
    func populationProfile(anim: Bool) {
        ...
        let maxCount = dataSource?.alongWrap(cardView: self) ?? 0
        ...
        for i in 0 ... showNumber - 1 {
            ...
            scaleOfMeasurementTransaction(index: i)
        }
    ...
    func scaleOfMeasurementTransaction(index: Int) {
        //: let cell = dataSource?.cardViewCellForRowAtIndex(cardView: self, index: index)
        let cell = dataSource?.foreword(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? MultiExtendView(), at: 0)
    ...
    func shineAnim(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: MultiExtendView = self.visibleCells[i] as! MultiExtendView
            ...
            if isVisbleLast {
                ...
                self.delegate?.arena?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                noSpark(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func noSpark(cell: MultiExtendView, transform: CGAffineTransform) {
    ...
    func codonStrength(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func shOpener(identifier: String) -> MultiExtendView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! NoView.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: NoView = cellclass.init(reuseIdentifier: identifier)
...
extension OutsideView: TransitionCellDelegete {
    ...
    func windowPersonCell(cell: MultiExtendView) {
        ...
        self.delegate?.compartmentIn?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.alongWrap(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.magneticDeclinationIndex?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            scaleOfMeasurementTransaction(index: self.currentIndex + 1)
        }
        // 更新布局
    ...
    func roll(cell: MultiExtendView, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.privacy?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
