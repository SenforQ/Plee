...
class ControllerThen: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: DotRelatableTarget?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.controlItem()
            }
        }
    ...
    func appearanceRoute() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.pullOn(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.smallColor()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.askNameColor(), NSAttributedString.Key.font: UIFont.shrinkSize(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.pullOn(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.askNameColor(), NSAttributedString.Key.font: UIFont.shrinkSize(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.askNameColor()
...
extension ControllerThen: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            ManagerThen.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
            //: tabBarController?.tabBar.isHidden = false
            tabBarController?.tabBar.isHidden = false
            //: TalkingAdvertisingManager.shared.advSuperView?.isHidden = false
            ManagerThen.shared.advSuperView?.isHidden = false
        }
    }
...
extension ControllerThen: UIGestureRecognizerDelegate {
