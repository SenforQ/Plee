...
fileprivate let show_behindKey:[UInt8] = [0xf3,0xf4,0xf3,0xee,0xb2,0xf9,0xf5,0xfe,0xff,0xe8,0xa0,0xb3,0xba,0xf2,0xfb,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xf8,0xff,0xff,0xf4,0xba,0xf3,0xf7,0xea,0xf6,0xff,0xf7,0xff,0xf4,0xee,0xff,0xfe]

private func timeDrag(remain num: UInt8) -> UInt8 {
    return num ^ 154
}

/*: "icon_me_voice_white" :*/
fileprivate let appCuteData:String = "permission read inform issueicon_me_"
fileprivate let userShouldRewardStr:[Character] = ["v","o","i","c","e","_","w","h","i","t","e"]

/*: "btn_auto_write" :*/
fileprivate let const_noticeDitKey:String = "btn_aseries home active"
fileprivate let showNumbPickData:[Character] = ["i","t","e"]

/*: "Pending review" :*/
fileprivate let data_warningId:String = "clothing minute on team yourselfPending"
fileprivate let userVoiceName:[Character] = [" ","r","e","v","i","e","w"]

/*: "get json error" :*/
fileprivate let mainImpressionFormat:String = "get jsonauto exception"
fileprivate let app_tunData:String = " "
fileprivate let kStarKey:[Character] = ["e","r","r","o","r"]
...
class ThanksAudioCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_behindKey.map{timeDrag(remain: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var statusLb: UILabel = {
        ...
        lb.textColor = UIColor.foursquare()
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var audioBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.thumbNumbererest(colors: UIColor.ratingConfirm(), size: CGSize(width: 155, height: 35)), for: .normal)
    ...
    lazy var playIconView: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.adjust(name: (String(appCuteData.suffix(8)) + String(userShouldRewardStr))))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var editBtn: UIButton = {
        ...
        btn.setImage(UIImage.adjust(name: (String(const_noticeDitKey.prefix(5)) + "uto_wr" + String(showNumbPickData))), for: .normal)
        //: btn.addTarget(self, action: #selector(editBtnAction), for: .touchUpInside)
        btn.addTarget(self, action: #selector(specs), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ThanksAudioCell {
    ...
    func limited(model: StanceHandyJSON, row: Int, isLast: Bool) {
        //: index = row
        index = row
        //: islast = isLast
        islast = isLast
        ...
        if model.status == 0 {
            ...
            statusLb.text = (String(data_warningId.suffix(7)) + String(userVoiceName)).localized
    ...
    func promulgation(isPlaying: Bool) {
    ...
    func day() {
        ...
        do {
            ...
            let url = PortraitJackEffectTool.default.responsibility(type: .Play_Audio_Wave)
            ...
            printLog(message: (String(mainImpressionFormat.prefix(8)) + app_tunData.capitalized + String(kStarKey)))
        }
    }
}
...
extension ThanksAudioCell {
    ...
    override func layoutSubviews() {
        ...
        if index == 0, islast {
            ...
            backView.currencyLess(width: backView.frame.size.width, height: backView.frame.size.height, corners: .allCorners, cornerRadii: .init(width: 8, height: 8))
            //: } else if index == 0 {
        } else if index == 0 {
            ...
            backView.currencyLess(width: backView.frame.size.width, height: backView.frame.size.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 8, height: 8))
            //: } else if islast {
        } else if islast {
            ...
            backView.currencyLess(width: backView.frame.size.width, height: backView.frame.size.height, corners: [.bottomLeft, .bottomRight], cornerRadii: .init(width: 8, height: 8))
            //: } else {
        } else {
            ...
            backView.currencyLess(width: backView.frame.size.width, height: backView.frame.size.height, corners: .allCorners, cornerRadii: .init(width: 0, height: 0))
        }
    }
}
