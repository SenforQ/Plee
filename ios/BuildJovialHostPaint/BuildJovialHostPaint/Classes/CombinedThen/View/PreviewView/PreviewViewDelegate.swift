...
fileprivate let main_conversionMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img_loading_lose" :*/
fileprivate let noti_computerNetComfortablePath:[Character] = ["i","m"]
fileprivate let userPleaseId:[Character] = ["g","_","l","o","a","d","i","n","g","_","l","o","s","e"]
...
class PreviewViewDelegate: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_conversionMessage.reversed(), encoding: .utf8)!)
    }
...
extension PreviewViewDelegate {
    //: private func bindInteraction() {
    private func enough() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(form))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(send)).then {
...
extension PreviewViewDelegate {
    ...
    func computerSimulationModel(imgModel: WidenMeasurable) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(noti_computerNetComfortablePath) + String(userPleaseId))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.makeRecover()
            }
        }
    ...
    func avenueStreetSmart(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(noti_computerNetComfortablePath) + String(userPleaseId))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.makeRecover()
        }
    }
...
extension PreviewViewDelegate: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension PreviewViewDelegate {
    ...
    func makeRecover() {
        ...
        var newW = dataModeBlockStr
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = dataModeBlockStr * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > constCornerDetailUrl {
            //: newH = ScreenHeight
            newH = constCornerDetailUrl
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * constCornerDetailUrl / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (dataModeBlockStr - newW) / 2.0, y: (constCornerDetailUrl - newH) / 2)
