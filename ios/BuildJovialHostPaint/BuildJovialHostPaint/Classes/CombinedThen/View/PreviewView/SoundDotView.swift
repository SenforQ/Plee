...
fileprivate let mainDimensionMsg:[UInt8] = [0x87,0x80,0x87,0x9a,0xc6,0x8d,0x81,0x8a,0x8b,0x9c,0xd4,0xc7,0xce,0x86,0x8f,0x9d,0xce,0x80,0x81,0x9a,0xce,0x8c,0x8b,0x8b,0x80,0xce,0x87,0x83,0x9e,0x82,0x8b,0x83,0x8b,0x80,0x9a,0x8b,0x8a]

private func tentFoundation(behavior num: UInt8) -> UInt8 {
    return num ^ 238
}

/*: "icon_male_default" :*/
fileprivate let kRequireData:[UInt8] = [0xdf,0xd5,0xd9,0xd8,0xe9,0xdb,0xd7,0xda,0xd3,0xe9,0xd2,0xd3,0xd0,0xd7,0xc3,0xda,0xc2]

/*: "icon_female_default" :*/
fileprivate let appAttentionKey:String = "holder interest rejecticon_"
fileprivate let show_languageStr:[Character] = ["l","e","_","d","e","f","a","u","l","t"]

/*: "level_%@" :*/
fileprivate let appTotManagerUrl:[Character] = ["l","e","v"]
fileprivate let data_brakeStr:String = "elect search evaluateel_%@"
...
class SoundDotView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainDimensionMsg.map{tentFoundation(behavior: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .shrinkSize(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .shrinkSize(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension SoundDotView {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func gameStreetSmart(model: PerceptInfoModel) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.writerExWho())
        ...
        let imgStr = model.sex == FullHashable.male.rawValue ? String(bytes: kRequireData.map{$0^182}, encoding: .utf8)! : (String(appAttentionKey.suffix(5)) + "fema" + String(show_languageStr))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(appTotManagerUrl) + String(data_brakeStr.suffix(5))), model.level)
