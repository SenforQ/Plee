...
fileprivate let const_bladeBurnData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#333333" :*/
fileprivate let user_togetherUrl:String = "#3333"
fileprivate let kProvinceMsg:String = "33"

/*: "My photos" :*/
fileprivate let main_traceWaveKey:[Character] = ["M","y"," ","p","h","o","t"]
fileprivate let constUpperValue:String = "acquisition"

/*: "#999999" :*/
fileprivate let data_exhibitDeadlinePath:[Character] = ["#","9","9","9","9","9","9"]

/*: "Click to change or delete photo." :*/
fileprivate let main_renderName:[UInt8] = [0xd5,0xfe,0xfb,0xf5,0xfd,0xb2,0x6,0x1,0xb2,0xf5,0xfa,0xf3,0x0,0xf9,0xf7,0xb2,0x1,0x4,0xb2,0xf6,0xf7,0xfe,0xf7,0x6,0xf7,0xb2,0x2,0xfa,0x1,0x6,0x1,0xc0]

fileprivate func activeMagnitude(scene num: UInt8) -> UInt8 {
...
fileprivate let notiBrightStr:String = "btn_mshow utilize"
fileprivate let data_beautyMsg:String = "violation answer life across electronicit_photo_"
fileprivate let constCancelTitle:[Character] = ["a","d"]
fileprivate let main_assertSafetyFormat:[Character] = ["d","_","n","o","r"]

/*: "btn_me_edit_photo_add_pre" :*/
fileprivate let appAverageContent:String = "patent"
fileprivate let const_whiteRootMsg:[Character] = ["n","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","a","d","d","_","p","r","e"]

/*: "btn_me_edit_photo_delete" :*/
fileprivate let userHeritageContent:[Character] = ["b","t","n","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","d","e","l","e"]
fileprivate let mainNorText:[Character] = ["t","e"]
...
public protocol OutsidePhotoDelegate: NSObject {
    /// 图片add
...
class EntryViewCell: UITableViewCell {
    ...
    open weak var delegate: OutsidePhotoDelegate?
...
//        showMessageView()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_bladeBurnData.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (user_togetherUrl.capitalized + kProvinceMsg.capitalized))
        //: lb.text = "My photos".localized
        lb.text = (String(main_traceWaveKey) + constUpperValue.replacingOccurrences(of: "acquisition", with: "os")).localized
    ...
    lazy var messageLB: UILabel = {
        ...
        lb.font = UIFont.shrinkSize(type: .Regular, fontSize: 12)
        //: lb.textColor = UIColor.init(hex: "#999999")
        lb.textColor = UIColor(hex: (String(data_exhibitDeadlinePath)))
        //: lb.text = "Click to change or delete photo.".localized
        lb.text = String(bytes: main_renderName.map{activeMagnitude(scene: $0)}, encoding: .utf8)!.localized
        //: return lb
        return lb
        //: }()
    }()
...
extension EntryViewCell {
    ...
    func tweedledeeAndTweedledum() {
        ...
        for i in 0 ..< 9 {
            ...
            colorBtn.setBackgroundImage(.adjust(name: (String(notiBrightStr.prefix(5)) + "e_ed" + String(data_beautyMsg.suffix(9)) + String(constCancelTitle) + String(main_assertSafetyFormat))), for: .normal)
            //: colorBtn.setBackgroundImage(.BundleImageNamed(name: "btn_me_edit_photo_add_pre"), for: .highlighted)
            colorBtn.setBackgroundImage(.adjust(name: (appAverageContent.replacingOccurrences(of: "patent", with: "bt") + String(const_whiteRootMsg))), for: .highlighted)
            ...
            colorBtn.rx.tap.subscribe(onNext: { [weak self] in
                ...
                self.playerSender(sender: colorBtn)
            ...
            deleteBtn.setBackgroundImage(.adjust(name: (String(userHeritageContent) + String(mainNorText))), for: .normal)
            ...
            deleteBtn.rx.tap.subscribe(onNext: { [weak self] in
                ...
                self.strongSender(sender: deleteBtn)
                //: }).disposed(by: disposeBag)
            }).disposed(by: disposeBag)
        }
    }
    ...
    @objc func playerSender(sender: UIButton) {
        //: var isReturn = false
        var isReturn = false
        ...
        GravityReactiveCompatible.albumAge(true) { (isOpen: Bool) in
            ...
            if isOpen {
                ...
                let vc = BeplasterPickTool.soundEarly(maxCount: 9 - self.count, allowPhoto: true, allowVideo: false)
                ...
                vc.didFinishPickingPhotosHandle = { [weak self] (photos: [UIImage]?,
                        ...
                        if photos!.count > 0 {
                            ...
                            self.delegate?.component(photos!)
                        }
                        //: return
                }
            }
    ...
    func compareSucceedSelete(selete: [WidenMeasurable]) {
        ...
        for i in 0 ..< selete.count {
            ...
            if i < allPhoto.count {
                ...
                if selete[i].isLocal {
                    ...
                    Btn.optimistic(urlStr: selete[i].url)
                }
                //: _ = Btn.subviews.map { view in
                _ = Btn.subviews.map { view in
                    //: if (view is UIButton) {
