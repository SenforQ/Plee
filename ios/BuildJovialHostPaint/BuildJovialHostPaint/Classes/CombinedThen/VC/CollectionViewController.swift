...
fileprivate let noti_napData:[Character] = ["i","c","o","n","_","k","o","n","g","_"]
fileprivate let dataMemoryKey:String = "KONG"
fileprivate let noti_totalGreenData:String = "_defaultjump illegal weight successfully upon"

/*: "page" :*/
fileprivate let app_vehicleGolfValue:[UInt8] = [0xdb,0xcc,0xd2,0xd0]

fileprivate func fieldEnjoy(shade num: UInt8) -> UInt8 {
...
/*: "uid" :*/
fileprivate let show_taName:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let mainEnterValue:String = "liinterestt"
...
class CollectionViewController: OperateReactiveCompatible {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [NoItemModel]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: kGreetData, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(RealmReusableView.self, forCellWithReuseIdentifier: RealmReusableView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.adjust(name: (String(noti_napData) + dataMemoryKey.lowercased() + String(noti_totalGreenData.prefix(8))))
...
extension CollectionViewController {
    ...
    private func listReplace() {
        //: let params = ["page": pageIndex, "uid": userInfoModel.uid] as [String: Any]
        let params = [String(bytes: app_vehicleGolfValue.map{fieldEnjoy(shade: $0)}, encoding: .utf8)!: pageIndex, String(bytes: show_taName.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        MomentRequestTool.bubble(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(mainEnterValue.replacingOccurrences(of: "interest", with: "s"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = WalkModelType.deserialize(from: dict as? Dictionary) {
...
extension CollectionViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: RealmReusableView.className(), for: indexPath) as! RealmReusableView
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = StuffVariableRecognizerDelegate(model: detailModel)
...
extension CollectionViewController: JXPagingViewListViewDelegate {
...
extension CollectionViewController {
    ...
    func quoteController(_ userModel: SeekTransformable) {
