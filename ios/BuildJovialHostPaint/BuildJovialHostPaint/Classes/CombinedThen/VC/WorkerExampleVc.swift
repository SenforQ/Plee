...
/*: "Face verification" :*/
fileprivate let k_narrowMessage:String = "Face spring nearby"
fileprivate let data_speakerLoseId:String = "expectedcatexpected"
fileprivate let k_finiteKey:[Character] = ["o","n"]
...
class WorkerExampleVc: OperateReactiveCompatible {
    ...
    override func viewDidLoad() {
        ...
        title = (String(k_narrowMessage.prefix(5)) + "verif" + data_speakerLoseId.replacingOccurrences(of: "expected", with: "i") + String(k_finiteKey)).localized
        //: self.createUI()
        self.assets()
    ...
    lazy var mainView: LawExampleView = {
        //: let view = TalkingVerificationExampleView()
        let view = LawExampleView()
        //: return view
        return view
        //: }()
    }()
...
extension WorkerExampleVc: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    //: private func takePhoto() {
    private func filterNoseClose() {
        ...
        self.dismiss(animated: true) {
            ...
            if let photo = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                ...
                if self.isRegisterPush {
                    ...
                    let vc = CompositionReactiveCompatible(photoImage: photo)
...
extension WorkerExampleVc {
    ...
    func assets() {
        ...
        mainView.btnClickBlock = { [weak self] () in
            ...
            GravityReactiveCompatible.takeWater(true) { isOpen in
                //: guard isOpen == true else { return }
                guard isOpen == true else { return }
                //: self.takePhoto()
                self.filterNoseClose()
            }
        }
    }
