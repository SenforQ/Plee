...
/*: "http://stat. :*/
fileprivate let data_lastHisPath:String = "http:/button low"
fileprivate let k_foundMessage:String = "/"
fileprivate let const_collectUrl:String = "write others postat."

/*: .com" :*/
fileprivate let notiWomanTitle:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let data_submitName:String = "http://margin layer huge"

/*: "https://" :*/
fileprivate let k_shareFormat:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let show_characterizeValue:String = "quote why angle meanhttp://m."

/*: "http:// :*/
fileprivate let show_searchKey:[Character] = ["h","t","t","p",":","/","/"]

/*: "3,90" :*/
fileprivate let show_collectionContent:[Character] = ["3",",","9","0"]

/*: "1990-01-01" :*/
fileprivate let user_networkKey:String = "1laylay0-01-0"
fileprivate let const_jumpUrl:String = "1"

/*: "MM-dd-yyyy" :*/
fileprivate let appAnalyzeHoldFloatFormat:[Character] = ["M","M","-","d","d"]
fileprivate let showObserveUrl:String = "-yyyypage sweet"
...
class ManhoodRequestModel: NSObject {
    ...
    var requestType: MisplacedRequestType?
    ...
    override init() {
        ...
        self.requestServer = CoverAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func telephoneSet() -> Bool {
...
struct IssueModelType: HandyJSON {
    //: var errno: Int!
...
public struct RunningErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct EvenCanvasHandyJSON: HandyJSON {
    ...
    var CSConfig = PromiseHandyJSON() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(data_lastHisPath.prefix(6)) + k_foundMessage.capitalized + String(const_collectUrl.suffix(5))) + "\(mainRequestMsg)" + (String(notiWomanTitle))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(data_submitName.prefix(7)))) && !statDomain.hasPrefix((String(k_shareFormat))) {
                //: return "http://".appending(statDomain)
                return (String(data_submitName.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(show_characterizeValue.suffix(9))) + "\(mainRequestMsg)" + (String(notiWomanTitle))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(show_searchKey)) + "\(urlDomain)"
        }
    }
...
public class AdequacyMeasurable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [TransitionChatPriceModel] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [TransitionChatPriceModel] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [TransitionChatPriceModel] = [] // 语音收费设置
    ...
    var vipSetting = [ModelGraphicMeasurable]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [BannerModel]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = CollectionModelType() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class LostTransformable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [SoulTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [SoulTagModel]?
    ...
    var callPopupSetting = (String(show_collectionContent)) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.seekEarly(self.birthday ?? (user_networkKey.replacingOccurrences(of: "lay", with: "9") + const_jumpUrl.capitalized), dateFormat: (String(appAnalyzeHoldFloatFormat) + String(showObserveUrl.prefix(5))))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.phoneStroke().year else { return "0" }
...
struct MethodMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct SoulTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func agal(compareTo: SoulTagModel) -> Bool {
        //: return
        return
...
struct PromiseMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [OptionTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [OptionTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct OptionTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [SoulTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct PromiseHandyJSON: HandyJSON {
...
struct ModelGraphicMeasurable: HandyJSON {
