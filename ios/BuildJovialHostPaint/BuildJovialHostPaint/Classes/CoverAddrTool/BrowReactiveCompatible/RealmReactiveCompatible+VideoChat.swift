...
/*: "type" :*/
fileprivate let data_assignName:String = "typequity"

/*: "You are currently on a call, please try again later" :*/
fileprivate let appNoStr:[UInt8] = [0xf6,0xc0,0xda,0x8f,0xce,0xdd,0xca,0x8f,0xcc,0xda,0xdd,0xdd,0xca,0xc1,0xdb,0xc3,0xd6,0x8f,0xc0,0xc1,0x8f,0xce,0x8f,0xcc,0xce,0xc3,0xc3,0x83,0x8f,0xdf,0xc3,0xca,0xce,0xdc,0xca,0x8f,0xdb,0xdd,0xd6,0x8f,0xce,0xc8,0xce,0xc6,0xc1,0x8f,0xc3,0xce,0xdb,0xca,0xdd]

/*: "onCallResume" :*/
fileprivate let k_unitId:String = "bag due us offer digonCall"
fileprivate let dataProgramLovelySoulName:String = "Resumetelephone provider past fun plus"
...
// __CLOSE_PRINT__
//
//  RealmReactiveCompatible+VideoChat.swift
//  AbroadTalking
//
...
extension RealmReactiveCompatible {
    ...
    func boxSituation(info: [String: Any], situation: MultiRoundChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(data_assignName.replacingOccurrences(of: "equity", with: "e"))].intValue
...
extension RealmReactiveCompatible {
    ...
    private func rangeLevel(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isFisher {
        if !resumeTalk && ManagerCommentThen.shared.isFisher {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.bringHome(showMsg: String(bytes: appNoStr.map{$0^175}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if currentPositionController()!.isKind(of: CrosswaysViewController.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            currentPositionController()!.isKind(of: ErrorPartyDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            currentPositionController()!.isKind(of: ThanksPlayerDelegate.self)
        {
            //: return
            return
        }
        ...
        /// 来电时，关闭男性邀请通话弹窗
        if (BrowKingfisherCompatible.shared as! BrowKingfisherCompatible).inviteCallView != nil {
            (BrowKingfisherCompatible.shared as! BrowKingfisherCompatible).inviteCallView?.closeView()
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! CrosswaysViewController).chatModel = CanvasChatModel.exceptSituation(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = ErrorPartyDelegate()
            ...
            (vc as! ErrorPartyDelegate).chatModel = CanvasChatModel.exceptSituation(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(k_unitId.suffix(6)) + String(dataProgramLovelySoulName.prefix(6)))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.directChannel(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func directChannel(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = currentPositionController() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = CurrencyDefineReactiveCompatible.getWindow().rootViewController as! UITabBarController
            let rootVc = CurrencyDefineReactiveCompatible.stripElement().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(ErrorPartyDelegate.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(CrosswaysViewController.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: CrosswaysViewController?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! CrosswaysViewController

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? CrosswaysViewController
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.broad(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: ControllerThen.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! ControllerThen
            ...
            var currentChatVc: ErrorPartyDelegate?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! ErrorPartyDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ErrorPartyDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.inmateOfThread(resumeData: resumeData)
                }
                //: return
                return
            }
