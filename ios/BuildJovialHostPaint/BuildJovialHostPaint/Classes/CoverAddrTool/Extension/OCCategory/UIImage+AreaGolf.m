...
#import "UIImage+AreaGolf.h"

//: @implementation UIImage (Compress)
@implementation UIImage (AreaGolf)

//: - (NSData *)compressedImageData {
- (NSData *)media {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.magnitudeRelation;
...
- (UIImage *)view {
    ...
    return [UIImage imageWithData:[self media]];
}
...
- (UIImage *)taphouse {
    ...
    return [self makeStop:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)magnitudeRelation {
    //: return [self wcCompres:NO];
    return [self makeStop:NO];
}
...
- (UIImage *)makeStop:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self isMargin:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self at:size];
...
- (CGSize)isMargin:(Boolean)isSession {
...
+ (NSData *)responseCommit:(UIImage *)image row:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image clearHandleGift:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG picShrink:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG exposure:scalesize dataConverter:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG picShrink:(fileSize*1024)];
    }
...
- (UIImage*)clearHandleGift:(CGSize)targetSize{
...
- (UIImage *)exposure:(CGSize)targetSize dataConverter:(BOOL)isScale{
...
- (NSData *)picShrink:(NSInteger)fileSize{
...
+ (UIImage *)part:(UIImage*)orginImage expected:(NSUInteger)level {
    // 获取BitmapData
