...
/*: "add_header_cookies" :*/
fileprivate let appFeeMsg:String = "add_delay people air doc orange"
fileprivate let mainTagDestroyData:String = "public publiclyr_co"
fileprivate let k_politicMessage:String = "okaddedes"
...
public extension UIImageView {
    ...
    @objc func noneAdvancedFinish(urlStr: String,
    ...
    internal func darkness(urlStr: String,
                           //: placeImg: UIImage = UIImage.placeImgSquare(),
                           placeImg: UIImage = UIImage.writerExWho(),
    ...
    @objc func perBy(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: noti_timeStr.noneVisitor(), with: mainRequestMsg)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.dataImage())
    }
    ...
    @objc func recordIn(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: CoverAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(appFeeMsg.prefix(4)) + "heade" + String(mainTagDestroyData.suffix(4)) + k_politicMessage.replacingOccurrences(of: "added", with: "i")))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.writerExWho(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
