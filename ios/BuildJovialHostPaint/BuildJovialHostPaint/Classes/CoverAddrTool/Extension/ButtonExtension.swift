...
/*: "recharge" :*/
fileprivate let show_wayTitle:String = "repiharg"
fileprivate let app_consequencePath:String = "reserve"

/*: "recharge_ :*/
fileprivate let kAnimalText:String = "song gumrecharge_"

/*: ".png" :*/
fileprivate let mainNearbyContent:String = "gift crop.png"

/*: .png" :*/
fileprivate let kFamilyName:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let show_ditText:String = ".jpgoval general move morning"

/*: .jpg" :*/
fileprivate let app_timingFatalName:String = "brake draft temporary.jpg"
...
enum LostImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class PostureTalkingButton: UIButton {
    ...
    var imageAlignment: LostImageAlignment = .left
...
extension UIButton {
    ...
    func numbererval(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        noConversation(interval: interval, topOffset: 0)
    }
    ...
    func noConversation(interval: CGFloat, topOffset: CGFloat) {
    ...
    func markSwitchset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func heritageSubmit(color: UIColor, forState: UIControl.State) {
    ...
    func optimistic(urlStr: String,
                    //: placeImg: UIImage? = UIImage.placeImgSquare(),
                    placeImg: UIImage? = UIImage.writerExWho(),
                    ...
                    loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: noti_timeStr.noneVisitor(), with: mainRequestMsg)
    ...
    func resolutionAlbum(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((show_wayTitle.replacingOccurrences(of: "pi", with: "c") + app_consequencePath.replacingOccurrences(of: "reserve", with: "e"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (show_wayTitle.replacingOccurrences(of: "pi", with: "c") + app_consequencePath.replacingOccurrences(of: "reserve", with: "e")), with: (String(kAnimalText.suffix(9))) + "\(user_workName)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.optimistic(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(mainNearbyContent.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(mainNearbyContent.suffix(4))), with: "_\(user_workName)" + (String(kFamilyName)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(show_ditText.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(show_ditText.prefix(4))), with: "_\(user_workName)" + (String(app_timingFatalName.suffix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.optimistic(urlStr: newUrl, placeImg: nil)
    }
