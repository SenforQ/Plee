...
/*: "RSA加密失败，str:  :*/
fileprivate let showSkipKey:String = "RSA加密失\u{8d25}\u{ff0c}"
fileprivate let k_extraMessage:String = "literal awarestr: "

/*: "weixin://" :*/
fileprivate let showVehicleUrl:String = "wecapxcapn"
fileprivate let notiDirectId:[Character] = [":","/","/"]

/*: "dingtalk://" :*/
fileprivate let mainCalculateKey:String = "DINGT"

/*: "lark://" :*/
fileprivate let mainSelectId:[Character] = ["l","a","r","k",":","/","/"]

/*: "wxwork://" :*/
fileprivate let mainEconomicUrl:[Character] = ["w","x","w","o","r","k",":","/","/"]

/*: "deviceId" :*/
fileprivate let noti_writerName:[UInt8] = [0x62,0x63,0x70,0x6f,0x65,0x63,0x4f,0x62]

private func bringLight(floor num: UInt8) -> UInt8 {
    return num ^ 6
}

/*: "weixin" :*/
fileprivate let appRecoverFadeEarPath:String = "weibitin"

/*: "wxwork" :*/
fileprivate let data_openingName:String = "wxwoscheme"

/*: "dingtalk" :*/
fileprivate let user_foodMsg:String = "sortin"
fileprivate let const_perspectiveKey:[Character] = ["g","t","a","l","k"]

/*: "lark" :*/
fileprivate let kHaveBeforeStr:String = "larsize"
...
extension UIDevice {
    ...
    class func cropBlock(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.wordInnerPrevueId())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func wordInnerPrevueId() -> String {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func everyTop(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (showSkipKey + String(k_extraMessage.suffix(5))) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (showVehicleUrl.replacingOccurrences(of: "cap", with: "i") + String(notiDirectId))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (mainCalculateKey.lowercased() + "alk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(mainSelectId))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(mainEconomicUrl))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(show_momentMessage) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: show_momentMessage)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = CoverAddrTool.unlessDevice() || CoverAddrTool.buryInService()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = everyTop(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: noti_writerName.map{bringLight(floor: $0)}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((appRecoverFadeEarPath.replacingOccurrences(of: "bit", with: "x")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((data_openingName.replacingOccurrences(of: "scheme", with: "rk")))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((user_foodMsg.replacingOccurrences(of: "sort", with: "d") + String(const_perspectiveKey)))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((kHaveBeforeStr.replacingOccurrences(of: "size", with: "k")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
