...
fileprivate let constCollectPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Cancel" :*/
fileprivate let kRelieveValue:String = "Cancelmax on stage resolution warn"

/*: "Sure" :*/
fileprivate let user_differenceValue:[Character] = ["S","u","r","e"]
...
class ObjectThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constCollectPath.reversed(), encoding: .utf8)!)
    }

    //: private func setupView() {
    private func lapsingStep() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(whiteInfo))
        ...
        cancelButton.setTitle((String(kRelieveValue.prefix(6))).localized, for: .normal)
        //: cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        cancelButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: cancelButton.setTitleColor(UIColor.black, for: .normal)
        cancelButton.setTitleColor(UIColor.black, for: .normal)
        //: cancelButton.addTarget(self, action: #selector(cancelClick(button:)), for: .touchUpInside)
        cancelButton.addTarget(self, action: #selector(morningShare(button:)), for: .touchUpInside)
        ...
        confirmButton.setTitle((String(user_differenceValue)).localized, for: .normal)
        //: confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        confirmButton.titleLabel?.font = UIFont.systemFont(ofSize: 16)
        //: confirmButton.setTitleColor(UIColor.black, for: .normal)
        confirmButton.setTitleColor(UIColor.black, for: .normal)
        //: confirmButton.addTarget(self, action: #selector(confirmClick(button:)), for: .touchUpInside)
        confirmButton.addTarget(self, action: #selector(consolidate(button:)), for: .touchUpInside)
    ...
    @objc func morningShare(button _: UIButton) {
        //: dismissView()
        whiteInfo()
    }

    //: @objc func confirmClick(button: UIButton) {
    @objc func consolidate(button _: UIButton) {
        //: dismissView()
        whiteInfo()
    }
