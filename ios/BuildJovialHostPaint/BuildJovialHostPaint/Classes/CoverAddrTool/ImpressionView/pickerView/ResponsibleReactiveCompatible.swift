...
/*: "Select a date" :*/
fileprivate let constNeverData:String = "opening elite front fullSelect "
fileprivate let show_pictureMsg:String = "luck"
fileprivate let appBagMessage:String = " datehand agency observer"
...
// __CLOSE_PRINT__
//
//  ResponsibleReactiveCompatible.swift
//  AbroadTalking
//
...
protocol InconsistentViewDelegate: NSObjectProtocol {
...
class ResponsibleReactiveCompatible: ObjectThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: InconsistentViewDelegate?

    //: override func initPickView() {
    override func theFeeView() {
        //: super.initPickView()
        super.theFeeView()

        //: titleString = "Select a date".localized
        titleString = (String(constNeverData.suffix(7)) + show_pictureMsg.replacingOccurrences(of: "luck", with: "a") + String(appBagMessage.prefix(5))).localized
        ...
        verticalDay(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func verticalDay(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func consolidate(button: UIButton) {
        //: super.confirmClick(button: button)
        super.consolidate(button: button)
        //: refreshPickViewData()
        headOf()
        ...
        if let day = selectDay {
            ...
            delegate?.orientation(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.orientation(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return daysIndependence(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func daysIndependence(year: Int, month: Int) -> Int {
