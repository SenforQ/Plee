...
/*: "Finish" :*/
fileprivate let constPoseData:String = "Finishtar operate till blank"

/*: "Cancel" :*/
fileprivate let app_networkUrl:String = "greatest across toleranceCancel"

/*: "btn_photo_choice_pre" :*/
fileprivate let main_succeedPopPath:String = "offer contact recognize stuffbtn_p"
fileprivate let kHoldKey:String = "oice_preskip attitude subject guide"

/*: "btn_photo_add_nor" :*/
fileprivate let noti_successfulUrl:String = "btn_phshrink day"
fileprivate let kDescriptionValue:[Character] = ["d","_","n","o","r"]

/*: "/tmp" :*/
fileprivate let notiOriginalFormat:String = "/tmpinform dot writer allow wave"

/*: "Export session failed" :*/
fileprivate let mainStateMessage:[Character] = ["E","x","p","o","r","t"," ","s","e","s","s","i","o","n"]
fileprivate let show_expressionTitle:String = "please leave deliver orientation for failed"

/*: "Export canceled" :*/
fileprivate let kDevoteId:[Character] = ["E","x","p","o","r","t"]
fileprivate let dataStraightStr:String = "turn draw so can"

/*: "Successful!" :*/
fileprivate let const_padMessage:[Character] = ["S","u","c","c","e","s","s"]
fileprivate let user_sunScreenTitle:String = "post fixedful!"

/*: "Unsupported video formats" :*/
fileprivate let main_widespreadMsg:String = "ear warning man additionalUn"
fileprivate let userNativeMinimumId:String = "suviewview"
fileprivate let appContainTitle:String = " videcurrently substance disk"
fileprivate let k_productMsg:String = "gravityats"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let data_illegalMessage:String = "conversation ex style produce chanceyyyy-"
fileprivate let notiBridgeHungTitle:String = "provider golf contact-HH:m"
fileprivate let userLocalFireOriginName:String = "hair"

/*: "/tmp/video- :*/
fileprivate let userCommentKey:String = "retain work attach sight/tmp/"
fileprivate let main_scopeValue:String = "video-makeup component contrast observer"

/*: .mp4" :*/
fileprivate let show_actionHeavyKey:[Character] = [".","m","p","4"]
...
class BeplasterPickTool: NSObject {
    ...
    class func soundEarly(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = dataModeBlockStr * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = dataModeBlockStr
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (constCornerDetailUrl - height) / 2, width: dataModeBlockStr, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(constPoseData.prefix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(app_networkUrl.suffix(6))).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.shrinkSize(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.destroyAppear()
        ...
        imagePicker?.naviTitleFont = UIFont.shrinkSize(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.askNameColor()
        ...
        imagePicker?.iconThemeColor = UIColor.labIndicator()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.adjust(name: (String(main_succeedPopPath.suffix(5)) + "hoto_ch" + String(kHoldKey.prefix(8))))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.adjust(name: (String(noti_successfulUrl.prefix(6)) + "oto_ad" + String(kDescriptionValue)))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.shrinkSize(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(constPoseData.prefix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(constPoseData.prefix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.labIndicator()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: dataModeBlockStr - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func completionPopStage(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(notiOriginalFormat.prefix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(notiOriginalFormat.prefix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(mainStateMessage) + String(show_expressionTitle.suffix(7))))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(kDevoteId) + String(dataStraightStr.suffix(4)) + "celed"))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(const_padMessage) + String(user_sunScreenTitle.suffix(4))))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(main_widespreadMsg.suffix(2)) + userNativeMinimumId.replacingOccurrences(of: "view", with: "p") + "orted" + String(appContainTitle.prefix(5)) + "o for" + k_productMsg.replacingOccurrences(of: "gravity", with: "m")))
            }
        }
    }
    ...
    private class func verticalVideo() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(data_illegalMessage.suffix(5)) + "MM-dd" + String(notiBridgeHungTitle.suffix(5)) + "m:ss-S" + userLocalFireOriginName.replacingOccurrences(of: "hair", with: "SS"))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(userCommentKey.suffix(5)) + String(main_scopeValue.prefix(6))) + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(show_actionHeavyKey)))
        //: return outputPath
        return outputPath
    }
}
