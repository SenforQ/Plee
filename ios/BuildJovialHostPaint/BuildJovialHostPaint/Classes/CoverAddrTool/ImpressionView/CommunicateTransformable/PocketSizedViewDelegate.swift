...
fileprivate let kDescriptionGestureDisplayId:[UInt8] = [0xc9,0xce,0xc9,0xd4,0x88,0xc3,0xcf,0xc4,0xc5,0xd2,0x9a,0x89,0x80,0xc8,0xc1,0xd3,0x80,0xce,0xcf,0xd4,0x80,0xc2,0xc5,0xc5,0xce,0x80,0xc9,0xcd,0xd0,0xcc,0xc5,0xcd,0xc5,0xce,0xd4,0xc5,0xc4]

fileprivate func turnCreateer(tumble num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: LiteralObjectSlideDirection) -> Void
...
class PocketSizedViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (userCoverGuideId - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kDescriptionGestureDisplayId.map{turnCreateer(tumble: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.morningWriter(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: ExcludeCropOverlay = {
        ...
        let overlay = ExcludeCropOverlay(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension PocketSizedViewDelegate {
    ...
    private func morningWriter(point: CGFloat) -> UIImage {
...
extension PocketSizedViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = userCoverGuideId - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.yaupShow(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.yaupShow(isShow: true)
    }
...
extension PocketSizedViewDelegate {
    ...
    private func pauseSubviews() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(userCoverGuideId)
