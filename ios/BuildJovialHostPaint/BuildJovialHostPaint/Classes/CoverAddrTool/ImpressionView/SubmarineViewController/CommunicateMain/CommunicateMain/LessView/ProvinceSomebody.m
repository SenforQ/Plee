...
typedef struct {
    Byte shrinkDeal;
    Byte *pair;
    unsigned int translationClear;
    Byte dynamicsSpillDown;
	int iconProfessional;
	int selfDestructOfficial;
} SmoothData;

NSString *StringFromSmoothData(SmoothData *data);


//: Height
SmoothData k_utilityName = (SmoothData){243, (Byte []){187, 150, 154, 148, 155, 135, 32}, 6, 212, 199, 229};

//: Width
SmoothData mainQualityWarningPath = (SmoothData){111, (Byte []){56, 6, 11, 27, 7, 172}, 5, 252, 188, 50};

//: 请打开相机权限
SmoothData showPreviouslyTitle = (SmoothData){12, (Byte []){228, 163, 187, 234, 133, 159, 233, 176, 140, 235, 151, 180, 234, 144, 182, 234, 145, 143, 229, 149, 156, 88}, 21, 187, 246, 10};
...
@interface ProvinceSomebody () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation ProvinceSomebody

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithCaptureHair:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        fromOnOutput:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   object:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         capacity:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self upshotPosition:iDevicePosition];
...
- (void)inAttitudeFrame:(CGPoint)point from:(CGRect)frame
{
    ...
    [self noneDisappear:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           sinceEnd:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                outsideResolutionRoot:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)noneDisappear:(AVCaptureFocusMode)focusMode sinceEnd:(AVCaptureExposureMode)exposureMode outsideResolutionRoot:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self capacity:^(AVCaptureDevice *captureDevice) {
...
- (void)capacity:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self upshotPosition:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self wait]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:StringFromSmoothData(&mainQualityWarningPath)] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:StringFromSmoothData(&k_utilityName)] floatValue];
...
- (CGRect)name:(CGRect)rect totalRelation:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:StringFromSmoothData(&mainQualityWarningPath)] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:StringFromSmoothData(&k_utilityName)] floatValue];
...
- (BOOL)wait
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:StringFromSmoothData(&showPreviouslyTitle)
...
- (AVCaptureDevice *)upshotPosition:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(rate:start:postConnexion:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate rate:captureOutput start:sampleBuffer postConnexion:connection];
        }
    }
...
Byte *SmoothDataToByte(SmoothData *data) {
    if (data->dynamicsSpillDown < 138) return data->pair;
    for (int i = 0; i < data->translationClear; i++) {
        data->pair[i] ^= data->shrinkDeal;
    }
    data->pair[data->translationClear] = 0;
    data->dynamicsSpillDown = 71;
	if (data->translationClear >= 2) {
		data->iconProfessional = data->pair[0];
		data->selfDestructOfficial = data->pair[1];
	}
    return data->pair;
}

NSString *StringFromSmoothData(SmoothData *data) {
    return [NSString stringWithUTF8String:(char *)SmoothDataToByte(data)];
}
