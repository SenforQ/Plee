...
typedef struct {
    Byte name;
    Byte *rise;
    unsigned int springRegion;
    Byte professionalDaily;
	int swingAround;
} PurchaseData;
...
//: STDisplayShader
PurchaseData const_campaignTitle = (PurchaseData){218, (Byte []){137, 142, 158, 179, 169, 170, 182, 187, 163, 137, 178, 187, 190, 191, 168, 16}, 15, 179, 82};
...
- (instancetype)initWithRenderReport:(CGRect)frame suite_strong:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self info:StringFromPurchaseData(&const_campaignTitle)
                //: fragmentShader:@"STDisplayShader"
                layer:StringFromPurchaseData(&const_campaignTitle)
                    //: forProgram:&stDisplayProgram];
                    narrow:&stDisplayProgram];
...
- (BOOL)info:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          layer:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              narrow:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self size:&vertexShader tap:0x8B31 markText:show_downTitle]) {
    ...
    if (![self size:&fragShader tap:0x8B30 markText:noti_nameValue]) {
    ...
    if (![self nearProgram:*programPointer]) {
...
- (BOOL)size:(GLuint *)shader tap:(GLenum)type markText:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
Byte *PurchaseDataToByte(PurchaseData *data) {
    if (data->professionalDaily < 129) return data->rise;
    for (int i = 0; i < data->springRegion; i++) {
        data->rise[i] ^= data->name;
    }
    data->rise[data->springRegion] = 0;
    data->professionalDaily = 85;
	if (data->springRegion >= 1) {
		data->swingAround = data->rise[0];
	}
    return data->rise;
}
