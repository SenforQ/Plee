...
struct MechanismWalkLabelOrigin {
    //: let x =  5.0
    let x = 5.0
    //: let y =  5.0
    let y = 5.0
...
class CanvasView: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        clout(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        clout(placeholder: nil, placeholderColor: nil)
    }
}
...
extension CanvasView {
    ...
    private func clout(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(MechanismWalkLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(MechanismWalkLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-MechanismWalkLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: LostContainerInset().top, left: LostContainerInset().left, bottom: LostContainerInset().bottom, right: LostContainerInset().right)
