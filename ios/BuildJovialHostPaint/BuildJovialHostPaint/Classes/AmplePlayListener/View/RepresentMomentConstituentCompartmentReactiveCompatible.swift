...
fileprivate let k_chooseMsg:[UInt8] = [0x32,0x37,0x32,0x3d,0xf1,0x2c,0x38,0x2d,0x2e,0x3b,0x3,0xf2,0xe9,0x31,0x2a,0x3c,0xe9,0x37,0x38,0x3d,0xe9,0x2b,0x2e,0x2e,0x37,0xe9,0x32,0x36,0x39,0x35,0x2e,0x36,0x2e,0x37,0x3d,0x2e,0x2d]

fileprivate func richRed(gum num: UInt8) -> UInt8 {
...
/*: "F5F5F5" :*/
fileprivate let const_betweenEntryInitialId:[Character] = ["F","5","F","5","F","5"]
...
// __CLOSE_PRINT__
//
//  RepresentMomentConstituentCompartmentReactiveCompatible.swift
//  AbroadTalking
//
...
class RepresentMomentConstituentCompartmentReactiveCompatible: UITableViewCell {
    ...
    var videoView = WorkerReactiveCompatible()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_chooseMsg.map{richRed(gum: $0)}, encoding: .utf8)!)
    }
    ...
    func domainCell(isListTableCell: Bool) {
    ...
    lazy var userInfoView: ThanksInfoView = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = ThanksInfoView()
    ...
    lazy var textContentView: LevelReactiveCompatible = {
        //: let  view = TalkingMomentTextContentView.init()
        let view = LevelReactiveCompatible()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var photoView: StuffScrollView = {
        ...
        let view = StuffScrollView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: OptionThen = {
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(const_betweenEntryInitialId)))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension RepresentMomentConstituentCompartmentReactiveCompatible {
    //: func configCell(model: TalkingMomentModel) {
    func area(model: WalkModelType) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.insiderInformationAcross(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.overName(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == CapabilityTextLiteral.Photo.rawValue || model.momentType == CapabilityTextLiteral.Live.rawValue {
...
//            }
            ...
            photoView.configTheoryModel(model: cunrrenModel)
...
//            }
            ...
            videoView.strokeTo(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.awaited(model: cunrrenModel)
...
//        }
    ...
    @objc func behindSumerrupt() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: cunrrenModel.videoInfo!.videoUrl)
        let vc = LevelViewDelegate(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != RealmReactiveCompatible.share.loginUserMode.sex {
        if cunrrenModel.sex != RealmReactiveCompatible.share.loginUserMode.sex {
    ...
    func gold() {
        ...
        if cunrrenModel.momentType == CapabilityTextLiteral.Video.rawValue {
...
extension RepresentMomentConstituentCompartmentReactiveCompatible {
    ...
    private func tool() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(behindSumerrupt))
