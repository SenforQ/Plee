...
fileprivate let dataCanPath:[UInt8] = [0x2b,0x2c,0x2b,0x36,0x6a,0x21,0x2d,0x26,0x27,0x30,0x78,0x6b,0x62,0x2a,0x23,0x31,0x62,0x2c,0x2d,0x36,0x62,0x20,0x27,0x27,0x2c,0x62,0x2b,0x2f,0x32,0x2e,0x27,0x2f,0x27,0x2c,0x36,0x27,0x26]

private func moreFatal(info num: UInt8) -> UInt8 {
    return num ^ 66
}
...
class ApproachReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataCanPath.map{moreFatal(info: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func towardRun() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(directEach))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(menu))
...
extension ApproachReusableView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func directEach(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func menu(tap: UIGestureRecognizer) {
    ...
    func combineUrl(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.noneAdvancedFinish(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func manual(image: UIImage) {
