...
/*: "%@ reply %@：%@" :*/
fileprivate let k_theFormat:[Character] = ["%","@"," ","r","e","p"]
fileprivate let main_feeName:String = "ly %@\u{ff1a}%@"
...
struct DocumentReplyModel: HandyJSON {
    //: var cid: String?
...
extension DocumentReplyModel {
    ...
    mutating func before() {
        ...
        if replyUser.count > 0 {
            ...
            tempContent = String(format: (String(k_theFormat) + main_feeName), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(StayTogetherSizeView.enableDoingce(text: tempContent, size: CGSize(width: dataModeBlockStr - 82, height: CGFloat(MAXFLOAT)), font: UIFont.shrinkSize(type: .Regular, fontSize: 14), lineNumber: 0).height)
