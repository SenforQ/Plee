...
struct WalkModelType: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct WarnModelType: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum CapabilityTextLiteral: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct AnyoneVideoModel: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: EntryWarnTitleConvertible?
}

///
//: enum MomentVideoStatus: String {
enum EntryWarnTitleConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension WalkModelType {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func modelTransfer(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(StayTogetherSizeView.enableDoingce(text: content!, size: CGSize(width: dataModeBlockStr - 54, height: CGFloat(MAXFLOAT)), font: UIFont.shrinkSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == CapabilityTextLiteral.Photo.rawValue || momentType == CapabilityTextLiteral.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
