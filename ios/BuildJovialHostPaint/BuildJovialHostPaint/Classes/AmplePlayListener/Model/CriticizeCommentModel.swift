...
/*: "reply" :*/
fileprivate let main_coordinatorId:String = "repalongy"
...
struct CriticizeCommentModel: HandyJSON {
    //: var cid: String?
    ...
    var comment = [DocumentReplyModel]()
...
extension CriticizeCommentModel {
    ...
    mutating func dado() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if RepresentLanguageManager.shared.direction == .leftToRight {
                ...
                tempContent = (main_coordinatorId.replacingOccurrences(of: "along", with: "l")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (main_coordinatorId.replacingOccurrences(of: "along", with: "l")).localized
            }
        ...
        let sizeheight = Int(StayTogetherSizeView.enableDoingce(text: tempContent, size: CGSize(width: dataModeBlockStr - 70, height: CGFloat(MAXFLOAT)), font: UIFont.shrinkSize(type: .Regular, fontSize: 16), lineNumber: 0).height)
