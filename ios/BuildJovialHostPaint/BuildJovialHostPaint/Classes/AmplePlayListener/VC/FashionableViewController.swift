...
/*: "tab" :*/
fileprivate let kTumbleOperationId:String = "tpress"

/*: "page" :*/
fileprivate let dataFactorKey:String = "PAGE"

/*: "uid" :*/
fileprivate let user_timeTempMsg:String = "urestore"

/*: "model" :*/
fileprivate let app_maxName:String = "moddocument"

/*: "There's no feed yet." :*/
fileprivate let user_listMessage:[Character] = ["T","h","e","r","e","\'","s"]
fileprivate let const_mediaKey:[Character] = [" ","n","o"," ","f","e","e","d"," ","y","e","t","."]
...
// __CLOSE_PRINT__
//
//  FashionableViewController.swift
//  AbroadTalking
//
...
class FashionableViewController: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var dataSourceArr: [TalkingMomentModel] = []
    var dataSourceArr: [WalkModelType] = []

    //: var seleteCell = TalkingMomentItemCell()
    var seleteCell = RepresentMomentConstituentCompartmentReactiveCompatible()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: const_hundredId, right: 0)
        //: return table
        return table
...
extension FashionableViewController {
    //: func reqData() {
    func clothingData() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(kTumbleOperationId.replacingOccurrences(of: "press", with: "ab"))] = 1
        //: dict["page"] = pageIndex
        dict[(dataFactorKey.lowercased())] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        MomentRequestTool.bring(params: dict) { succeed, result, errorModel in
            ...
            if succeed {
                ...
                for i in 0 ..< array.count {
                    ...
                    if var model = JSONDeserializer<WalkModelType>.deserializeFrom(dict: array[i] as? [String: Any], designatedPath: nil) {
...
extension FashionableViewController {
    ...
    @objc func lineNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(user_timeTempMsg.replacingOccurrences(of: "restore", with: "id"))] as! String
    ...
    @objc func agree(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: WalkModelType = userinfo[(app_maxName.replacingOccurrences(of: "document", with: "el"))] as! WalkModelType
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func omitnotifBridgeNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: WalkModelType = userinfo[(app_maxName.replacingOccurrences(of: "document", with: "el"))] as! WalkModelType
...
extension FashionableViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = RepresentMomentConstituentCompartmentReactiveCompatible.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? RepresentMomentConstituentCompartmentReactiveCompatible
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = RepresentMomentConstituentCompartmentReactiveCompatible(style: .default, reuseIdentifier: identifier)
        }
        //: let model: TalkingMomentModel = self.dataSourceArr[indexPath.row]
        let model: WalkModelType = self.dataSourceArr[indexPath.row]
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == CapabilityTextLiteral.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: RepresentMomentConstituentCompartmentReactiveCompatible = cell as? RepresentMomentConstituentCompartmentReactiveCompatible else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = StuffVariableRecognizerDelegate(model: model)
...
extension FashionableViewController: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        running(scrollView: scrollView)
    }
    ...
    func running(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: RepresentMomentConstituentCompartmentReactiveCompatible = mainTableView.cellForRow(at: middleIndexPath) as! RepresentMomentConstituentCompartmentReactiveCompatible
...
extension FashionableViewController {
    ...
    private func exAccelerateVisual() {
        ...
        style.TipsTitle = (String(user_listMessage) + String(const_mediaKey)).localized
        ...
        mainTableView.register(RepresentMomentConstituentCompartmentReactiveCompatible.self, forCellReuseIdentifier: RepresentMomentConstituentCompartmentReactiveCompatible.className())
    ...
    private func askTotaleraction() {
        //: mainTableView.addHeaderRefresh { [weak self] in
        mainTableView.mixInEraseComplectionTrack { [weak self] in
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(agree(notification:)), name: user_managerId, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(lineNotification(notification:)), name: data_networkMsg, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(omitnotifBridgeNotification(notification:)), name: appAgentPath, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(exceptMasterGreet), name: k_showTimeKey, object: nil)
    }
}
