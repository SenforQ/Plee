...
fileprivate let kInjuryUrl:[UInt8] = [0x96,0x91,0x96,0x8b,0xd7,0x9c,0x90,0x9b,0x9a,0x8d,0xc5,0xd6,0xdf,0x97,0x9e,0x8c,0xdf,0x91,0x90,0x8b,0xdf,0x9d,0x9a,0x9a,0x91,0xdf,0x96,0x92,0x8f,0x93,0x9a,0x92,0x9a,0x91,0x8b,0x9a,0x9b]

private func countoGoing(air num: UInt8) -> UInt8 {
    return num ^ 255
}

/*: "Save successfully" :*/
fileprivate let const_adviceGirlTitle:[Character] = ["S","a","v","e"," ","s","u","c","c"]
fileprivate let showCapabilityMsg:[Character] = ["e","s","s","f","u","l","l","y"]
...
class CropFlowLayout: TranslationReactiveCompatible {
    ...
    init(momentModel: WalkModelType, index: Int, type: GravitationalForcePhotoType) {
        ...
        if RepresentLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(dataModeBlockStr), y: 0)
    ...
    init(images: [UIImage], index: Int, type: GravitationalForcePhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kInjuryUrl.map{countoGoing(air: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: dataModeBlockStr, height: constCornerDetailUrl)
        ...
        view.frame = CGRect(x: 0, y: 0, width: dataModeBlockStr, height: constCornerDetailUrl)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(dataModeBlockStr), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(dataModeBlockStr), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(dataModeBlockStr), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(ApproachReusableView.self, forCellWithReuseIdentifier: ApproachReusableView.className())
...
//    lazy var downloadBtn: PostureTalkingButton = {
//        let btn = PostureTalkingButton.init()
...
//        GravityReactiveCompatible.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: WarnModelType = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func remainImage(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.readingDownMsg(showMsg: (String(const_adviceGirlTitle) + String(showCapabilityMsg)).localized)
                }
                //: } else {
            } else {
...
extension CropFlowLayout: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func aboard() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func small() {
        //: if  currenModel.sex==RealmReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == RealmReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: ApproachReusableView = collectionView.dequeueReusableCell(withReuseIdentifier: ApproachReusableView.className(), for: indexPath) as! ApproachReusableView
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: WarnModelType = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.combineUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.manual(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.manual(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: ApproachReusableView = cell as! ApproachReusableView
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / dataModeBlockStr)
        //: if myType == .normal {
        if myType == .normal {
