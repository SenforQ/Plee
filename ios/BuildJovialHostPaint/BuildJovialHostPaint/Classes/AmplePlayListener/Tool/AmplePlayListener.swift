...
/*: "/TXCache" :*/
fileprivate let constThemPatentTitle:String = "writing mobile protocol dialog/TXCach"
fileprivate let user_usedHaulUrl:String = "core"
...
protocol EntrywayReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func statusWill(player: AmplePlayListener, status: PatternPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func tabStep(player: AmplePlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func writeChoice(player: AmplePlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class AmplePlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: EntrywayReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: PatternPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = AmplePlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(constThemPatentTitle.suffix(7)) + user_usedHaulUrl.replacingOccurrences(of: "core", with: "e"))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(pass), userInfo: nil, repeats: true)
...
extension AmplePlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func midnight(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: noti_timeStr.noneVisitor(), with: mainRequestMsg)
    ...
    @objc func pass() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.writeChoice(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func spaceMode(renderMode: TransitionRenderMode) {
    ...
    func appShould(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isFisher == true {
        if ManagerCommentThen.shared.isFisher == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func between() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.statusWill(player: self, status: self.playStatus!)
        }
    }
...
extension AmplePlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.tabStep(player: self, duration: Int(duration), currentTime: Int(currentTime))
