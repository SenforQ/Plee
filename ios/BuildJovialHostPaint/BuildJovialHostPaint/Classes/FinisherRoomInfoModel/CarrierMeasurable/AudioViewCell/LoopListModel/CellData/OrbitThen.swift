...
/*: "title" :*/
fileprivate let app_computeAtFormat:[Character] = ["t","i","t","l","e"]

/*: "content" :*/
fileprivate let const_signatureName:[Character] = ["c","o","n","t","e","n","t"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiPrimaryData:[UInt8] = [0x6e,0x69,0x6e,0x73,0x2f,0x64,0x68,0x63,0x62,0x75,0x3d,0x2e,0x27,0x6f,0x66,0x74,0x27,0x69,0x68,0x73,0x27,0x65,0x62,0x62,0x69,0x27,0x6e,0x6a,0x77,0x6b,0x62,0x6a,0x62,0x69,0x73,0x62,0x63]

private func islandColor(leadership num: UInt8) -> UInt8 {
    return num ^ 7
}

/*: "img" :*/
fileprivate let notiEquipmentMsg:String = "imfrom"

/*: "jumpKey" :*/
fileprivate let data_dateVersionText:String = "jumpKeyline tun yourself"
...
@objcMembers public class OrbitThen: PinReactiveCompatible {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.askNameColor(), .font: UIFont.shrinkSize(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(String(app_computeAtFormat))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.destroyAppear(), .font: UIFont.shrinkSize(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(String(const_signatureName))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiPrimaryData.map{islandColor(leadership: $0)}, encoding: .utf8)!)
    }
    ...
    override public func width(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(notiEquipmentMsg.replacingOccurrences(of: "from", with: "g"))].stringValue.count > 0 {
        ...
        if extraJson[(String(app_computeAtFormat))].stringValue.count > 0 {
        ...
        if extraJson[(String(const_signatureName))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(data_dateVersionText.prefix(7)))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
