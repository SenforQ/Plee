...
fileprivate let show_remarkId:[UInt8] = [0x24,0x29,0x24,0x2f,0xe3,0x1e,0x2a,0x1f,0x20,0x2d,0xf5,0xe4,0xdb,0x23,0x1c,0x2e,0xdb,0x29,0x2a,0x2f,0xdb,0x1d,0x20,0x20,0x29,0xdb,0x24,0x28,0x2b,0x27,0x20,0x28,0x20,0x29,0x2f,0x20,0x1f]

fileprivate func characterizeUp(adjust num: UInt8) -> UInt8 {
...
/*: "icon_chat_miss" :*/
fileprivate let k_bubbleOpeningText:String = "iagain"
fileprivate let showHeartMsg:String = "favorite hold complyn_cha"

/*: "icon_chat_yes" :*/
fileprivate let noti_styleDonResolutionFormat:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let notiAidCurveKey:String = "ypulls"
...
class MaterialRealmMsgCell: FamilySeekCompartmentMsgCell {
//    var transBlock: (() -> Void)?

    //: var textData: ExternalPinCellData?
    var textData: ExternalPinCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_remarkId.map{characterizeUp(adjust: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.ditMention(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    override func beyondDestroy(with data: ThanksNoCellData) {
        ...
        if let temTextData = data as? ExternalPinCellData {
            ...
            if imgPreview.count > 0, temTextData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
                ...
                giftImagV.noneAdvancedFinish(urlStr: imgPreview)
            }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != CurrencyDefineReactiveCompatible.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.outgrowth() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.adjust(name: (k_bubbleOpeningText.replacingOccurrences(of: "again", with: "co") + String(showHeartMsg.suffix(5)) + "t_miss"))
                ...
                self.transStatusImagV.image = UIImage.adjust(name: (String(noti_styleDonResolutionFormat) + notiAidCurveKey.replacingOccurrences(of: "pull", with: "e")))
            }
    ...
    func observerGes() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(adminComplete(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == CurrencyDefineReactiveCompatible.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == CurrencyDefineReactiveCompatible.metricLinearUnit())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            feedWhen()
...
//            }
    ...
    func feedWhen() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(fertilePeriodPost(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func adminComplete(tap _: UITapGestureRecognizer?) {
    ...
    @objc func fertilePeriodPost(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
