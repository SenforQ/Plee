...
fileprivate let data_retainId:[UInt8] = [0x84,0x89,0x84,0x8f,0x43,0x7e,0x8a,0x7f,0x80,0x8d,0x55,0x44,0x3b,0x83,0x7c,0x8e,0x3b,0x89,0x8a,0x8f,0x3b,0x7d,0x80,0x80,0x89,0x3b,0x84,0x88,0x8b,0x87,0x80,0x88,0x80,0x89,0x8f,0x80,0x7f]

fileprivate func transferGuidance(scan num: UInt8) -> UInt8 {
...
/*: "move" :*/
fileprivate let dataObserverAlbumEditMessage:String = "movperspective"
...
class PortraitScreenView: UIView {
    ...
    var danumuMsgArray: [ThanksScreenModel] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [HoweverItemView] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ThanksScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_retainId.map{transferGuidance(scan: $0)}, encoding: .utf8)!)
    }
    ...
    func theFailReceive(_ danumuMsgModel: ThanksScreenModel?) {
    ...
    @objc func context(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(context(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension PortraitScreenView: UIGestureRecognizerDelegate {
    ...
    func unwelcomeItemBuildPremiereView(for danumuMsgModel: ThanksScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = HoweverItemView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: dataModeBlockStr, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if RepresentLanguageManager.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = dataModeBlockStr
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (dataObserverAlbumEditMessage.replacingOccurrences(of: "perspective", with: "e")))
...
//    }
    ...
    func licenseEditor(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                unwelcomeItemBuildPremiereView(for: model, isFirst: isFirst)
...
extension PortraitScreenView {
    // 添加视图
    //: private func setupSubviews() {
    private func noSequence() {
