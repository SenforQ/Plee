...
fileprivate let showWithoutFormat:[UInt8] = [0x36,0x31,0x36,0x2b,0x77,0x3c,0x30,0x3b,0x3a,0x2d,0x65,0x76,0x7f,0x37,0x3e,0x2c,0x7f,0x31,0x30,0x2b,0x7f,0x3d,0x3a,0x3a,0x31,0x7f,0x36,0x32,0x2f,0x33,0x3a,0x32,0x3a,0x31,0x2b,0x3a,0x3b]

/*: "View read receipts?" :*/
fileprivate let app_personalMsg:[Character] = ["V","i","e","w"," ","r","e","a","d"," ","r","e","c","e","i","p","t","s","?"]

/*: "text" :*/
fileprivate let user_memoryId:String = "treceiverxt"

/*: "color" :*/
fileprivate let showCurveValue:[UInt8] = [0x9b,0x97,0x94,0x97,0x8a]

private func lawOfTheLand(nor num: UInt8) -> UInt8 {
    return num ^ 248
}

/*: "<xml>%@</xml>" :*/
fileprivate let userComputerHundredFormat:String = "appropriate collection increase favorite<xml>%@"
fileprivate let main_writingStr:String = "some he coat</xml>"

/*: "href" :*/
fileprivate let kConversationName:String = "HREF"

/*: "type" :*/
fileprivate let data_boyBusyText:[UInt8] = [0xb7,0xbc,0xb3,0xa8]

fileprivate func harassmentService(aircraft num: UInt8) -> UInt8 {
...
/*: "txt" :*/
fileprivate let main_middleValue:[Character] = ["t","x","t"]

/*: "padding" :*/
fileprivate let k_chinId:String = "paddilibraryg"

/*: "size" :*/
fileprivate let const_judgeUrl:String = "sibehaviore"

/*: "maxFont" :*/
fileprivate let showSlimUpperName:String = "maxFoboy class"
fileprivate let dataSourceKey:String = "NT"

/*: "link" :*/
fileprivate let show_keepPresentId:String = "lidelayk"

/*: "PingFangSC-Regular" :*/
fileprivate let data_nothingStr:String = "economic cooperativePingFa"
fileprivate let const_arrayTitle:[Character] = ["-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let userPackId:[UInt8] = [0x81,0x8d,0x8c,0x96,0x87,0x8c,0x96]

private func soonAnimal(forehead num: UInt8) -> UInt8 {
    return num ^ 226
}
...
@objcMembers public class TrivialityThen: PinReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showWithoutFormat.map{$0^95}, encoding: .utf8)!)
    }

    //: public override func heightOfWidth(_ width: CGFloat) ->CGFloat {
    override public func width(_: CGFloat) -> CGFloat {
        //: return contentSize().height+20
        return conceal().height + 20
    }
    ...
    override public func conceal() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(app_personalMsg)).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: dataModeBlockStr, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: CurrencyDefineReactiveCompatible.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: CurrencyDefineReactiveCompatible.rawPara() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = tar(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return report(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func report(tips: String) -> NSMutableAttributedString {
        ...
        _ = script(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: CurrencyDefineReactiveCompatible.getScreenWidth()-50, para: para)
        self.yyLayout = expectedPara(array: attrbutedArray, displayMaxWidth: CurrencyDefineReactiveCompatible.rawPara() - 50, para: para)
    ...
    func script(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = on(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(user_memoryId.replacingOccurrences(of: "receiver", with: "e"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!, (user_memoryId.replacingOccurrences(of: "receiver", with: "e"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = tarShow(content: text, color: textColor)
    ...
    func on(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(userComputerHundredFormat.suffix(7)) + String(main_writingStr.suffix(6))), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            calledMap(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(user_memoryId.replacingOccurrences(of: "receiver", with: "e")): newSource, String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func calledMap(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(kConversationName.lowercased())] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(kConversationName.lowercased())] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                calledMap(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(user_memoryId.replacingOccurrences(of: "receiver", with: "e"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func expectedPara(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = paragraph(array: array, paragraphStyle: para)
    ...
    func paragraph(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: data_boyBusyText.map{harassmentService(aircraft: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (String(main_middleValue)) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = weight(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (k_chinId.replacingOccurrences(of: "library", with: "n")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(const_judgeUrl.replacingOccurrences(of: "behavior", with: "z"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(showSlimUpperName.prefix(5)) + dataSourceKey.lowercased())] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!] as? UIColor)
    ...
    func anonymous(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(const_judgeUrl.replacingOccurrences(of: "behavior", with: "z"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(showSlimUpperName.prefix(5)) + dataSourceKey.lowercased())] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func weight(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: data_boyBusyText.map{harassmentService(aircraft: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (show_keepPresentId.replacingOccurrences(of: "delay", with: "n")) && type != (String(main_middleValue)) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = streetwise()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(const_judgeUrl.replacingOccurrences(of: "behavior", with: "z"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(data_nothingStr.suffix(6)) + "ngSC" + String(const_arrayTitle)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: userPackId.map{soonAnimal(forehead: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: userPackId.map{soonAnimal(forehead: $0)}, encoding: .utf8)!] : ""
    ...
    func streetwise() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.ditMention(fontSize: 15)
    ...
    func tarShow(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return notMargin(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: data_boyBusyText.map{harassmentService(aircraft: $0)}, encoding: .utf8)!: (String(main_middleValue)),
                //: "content": content,
                String(bytes: userPackId.map{soonAnimal(forehead: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: showCurveValue.map{lawOfTheLand(nor: $0)}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func notMargin(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
