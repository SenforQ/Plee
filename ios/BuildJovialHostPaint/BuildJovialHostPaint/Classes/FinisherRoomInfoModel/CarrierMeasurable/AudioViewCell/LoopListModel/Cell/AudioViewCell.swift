...
fileprivate let userMatterMsg:[UInt8] = [0xb2,0xb5,0xb2,0xaf,0xf3,0xb8,0xb4,0xbf,0xbe,0xa9,0xe1,0xf2,0xfb,0xb3,0xba,0xa8,0xfb,0xb5,0xb4,0xaf,0xfb,0xb9,0xbe,0xbe,0xb5,0xfb,0xb2,0xb6,0xab,0xb7,0xbe,0xb6,0xbe,0xb5,0xaf,0xbe,0xbf]

private func givenSearch(paper num: UInt8) -> UInt8 {
    return num ^ 219
}

/*: "icon_chat_translate_nor" :*/
fileprivate let kPermissionKey:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let const_limitedKey:[Character] = ["t","_","t","r","a","n","s","l","a","t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let noti_charmTrustBanTitle:String = "termcon"
fileprivate let data_customerValue:String = "discount arrowt_tr"
fileprivate let main_midnightData:String = "_prerepresentation random"

/*: "icon_chat_miss" :*/
fileprivate let main_bookSweetId:String = "icawake"
fileprivate let noti_missingId:[Character] = ["t","_","m","i","s","s"]

/*: "icon_chat_yes" :*/
fileprivate let const_watchHornStr:String = "facility rank take incident scoreicon_"
fileprivate let userHaveStr:String = "ydisks"
...
class AudioViewCell: FamilySeekCompartmentMsgCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: CollectionCellData?
    var textData: CollectionCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userMatterMsg.map{givenSearch(paper: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.adjust(name: (String(kPermissionKey) + String(const_limitedKey))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.adjust(name: (noti_charmTrustBanTitle.replacingOccurrences(of: "term", with: "i") + "_cha" + String(data_customerValue.suffix(4)) + "anslate" + String(main_midnightData.prefix(4)))), for: .highlighted)
...
extension AudioViewCell {
    ...
    override func beyondDestroy(with data: ThanksNoCellData) {
        ...
        //: let maxWidth = CurrencyDefineReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = CurrencyDefineReactiveCompatible.rawPara() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.adjust(name: (main_bookSweetId.replacingOccurrences(of: "awake", with: "on") + "_cha" + String(noti_missingId)))
            ...
            self.transStatusImagV.image = UIImage.adjust(name: (String(const_watchHornStr.suffix(5)) + "chat_" + userHaveStr.replacingOccurrences(of: "disk", with: "e")))
        }
    ...
    func usance() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(compartmentNumbererrupt(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func compartmentNumbererrupt(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
