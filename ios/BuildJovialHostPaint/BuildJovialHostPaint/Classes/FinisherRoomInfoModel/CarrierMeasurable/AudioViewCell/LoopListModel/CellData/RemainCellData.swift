...
fileprivate let user_frameStr:[UInt8] = [0x1f,0x18,0x1f,0x2,0x5e,0x15,0x19,0x12,0x13,0x4,0x4c,0x5f,0x56,0x1e,0x17,0x5,0x56,0x18,0x19,0x2,0x56,0x14,0x13,0x13,0x18,0x56,0x1f,0x1b,0x6,0x1a,0x13,0x1b,0x13,0x18,0x2,0x13,0x12]

private func starItem(transition num: UInt8) -> UInt8 {
    return num ^ 118
}

/*: "Send to %@" :*/
fileprivate let kOurFormat:String = "Send tar translation hello"
fileprivate let userRearTitle:String = "recommendation forth littleto %@"

/*: "all" :*/
fileprivate let notiTingStr:[Character] = ["a","l","l"]

/*: "Send to All (Total %@)" :*/
fileprivate let dataBeautyEarlyId:String = "nature personal happy meeting matchSend t"
fileprivate let app_insertKeepName:String = "(Totaequipment address network"
fileprivate let noti_communicationData:[Character] = ["l"," ","%","@",")"]

/*: "#FFEE4B" :*/
fileprivate let noti_temporaryUrl:String = "all king independence recognize sense#FFEE"
fileprivate let show_deliverFormat:String = "4Bmotion both incorporate resume assign"
...
@objcMembers public class RemainCellData: PinReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_frameStr.map{starItem(transition: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        let attr = clickFormat(giftModel: self.msgModel)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func clickFormat(giftModel: ProLipModel) -> NSMutableAttributedString {
        ...
        let giftname = giftmodelWith(giftmodel: giftModel.gift)
        //: var str = "Send to %@".localizedArguments(giftModel.toUser.nickname)
        var str = (String(kOurFormat.prefix(5)) + String(userRearTitle.suffix(5))).restateArguments(giftModel.toUser.nickname)
        //: if giftModel.toUser.uid == "all" { // 发送所有人礼物, nickname只有数量
        if giftModel.toUser.uid == (String(notiTingStr)) { // 发送所有人礼物, nickname只有数量
            //: str = "Send to All (Total %@)".localizedArguments(giftModel.toUser.nickname)
            str = (String(dataBeautyEarlyId.suffix(6)) + "o All " + String(app_insertKeepName.prefix(5)) + String(noti_communicationData)).restateArguments(giftModel.toUser.nickname)
        }
        ...
        let color2 = (self.direction == .MsgDirectionOutgoing ? UIColor(hex: (String(noti_temporaryUrl.suffix(5)) + String(show_deliverFormat.prefix(2)))) : UIColor.labIndicator())
    ...
    private func giftmodelWith(giftmodel: DigModel) -> String {
