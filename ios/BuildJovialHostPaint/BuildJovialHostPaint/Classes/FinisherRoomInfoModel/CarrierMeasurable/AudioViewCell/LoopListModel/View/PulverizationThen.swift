...
fileprivate let notiBalanceKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chats_banner_subtract" :*/
fileprivate let noti_dismissMsg:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b"]
fileprivate let data_handPairStr:String = "not take shot shake scoreanner_su"
fileprivate let notiWellErrorUrl:[Character] = ["b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let showExpectedIllegalListText:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u"]
fileprivate let user_effDonStr:[Character] = ["b"]
fileprivate let userMustName:[Character] = ["t","r","a","c","t","_","p","r","e"]

/*: "bannerList" :*/
fileprivate let constShallValue:String = "bproducenne"
fileprivate let user_excitementMsg:[Character] = ["r","L","i","s","t"]

/*: "JXBannerCellID" :*/
fileprivate let notiRetPath:String = "receiver production stay richJXBanner"
fileprivate let showBeautyTitle:String = "CellIDagree will hand into double"

/*: "#FFFFFF" :*/
fileprivate let appOppositeTitle:[Character] = ["#","F","F","F","F","F","F"]
...
protocol ThighReactiveCompatible: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func putInView(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class PulverizationThen: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: ThighReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiBalanceKey.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [BannerModel] = //: return Array<BannerModel>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.adjust(name: (String(noti_dismissMsg) + String(data_handPairStr.suffix(8)) + String(notiWellErrorUrl))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.adjust(name: (String(showExpectedIllegalListText) + String(user_effDonStr) + String(userMustName))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(revenueClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension PulverizationThen {
    /// banner
    //: func reqBanner() {
    func tapResultSeekDraft() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        ErrorRequestManager().yearReplacementCompletion(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(constShallValue.replacingOccurrences(of: "produce", with: "a") + String(user_excitementMsg))] ?? []).arrayValue
...
extension PulverizationThen {
    ...
    @objc private func revenueClick() {
        ...
        self.delegate?.putInView(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func crowding(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension PulverizationThen: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(notiRetPath.suffix(8)) + String(showBeautyTitle.prefix(6))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.darkness(urlStr: url, placeImg: UIImage.writerExWho())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(appOppositeTitle)))!
