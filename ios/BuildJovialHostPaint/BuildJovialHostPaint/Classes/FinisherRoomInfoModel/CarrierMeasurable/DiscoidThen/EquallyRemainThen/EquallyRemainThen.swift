...
fileprivate let show_nobodyExFormat:String = "build allow suite projectCacheGi"
fileprivate let data_gravityUrl:[Character] = ["f","t","E","f","f","e","c","t","C","a"]
fileprivate let user_laterConvertValue:String = "ample array voicecheKey"

/*: "下载动画文件" :*/
fileprivate let kFeeContent:String = "下载\u{52a8}画文件"

/*: "加载本地动画文件" :*/
fileprivate let user_storageData:String = "加载本地"

/*: "Cache_GiftEffect/" :*/
fileprivate let noti_keepPath:[Character] = ["C","a","c","h","e","_","G","i","f","t","E","f","f","e","c","t","/"]

/*: "GJSGVA" :*/
fileprivate let kExpectedData:[Character] = ["G"]
fileprivate let notiCookieScanMessage:[Character] = ["J","S","G","V","A"]

/*: "GJVap" :*/
fileprivate let appLovelyBeyondData:String = "leadership match challenge plain mapGJVap"

/*: "config.json" :*/
fileprivate let const_heavyTriggerText:String = "cpausen"
fileprivate let dataBroadMessage:String = "fig.min relation ply shall"

/*: "app. :*/
fileprivate let notiStickKey:String = "fatal smooth onlyapp."

/*: .com" :*/
fileprivate let constMovieTitle:String = ".comthem origin injury"

/*: "key" :*/
fileprivate let dataCostId:String = "kresistance"

/*: "zip" :*/
fileprivate let user_holdStatusDateId:String = "zworkerp"

/*: " fail to unzip file" :*/
fileprivate let const_keepId:String = " fail leading process second"
fileprivate let kFirstCommitData:String = "fstandle"

/*: "RSA 解密失败" :*/
fileprivate let const_denyBringLoopId:String = "RSA \u{89e3}密失"
fileprivate let user_transformUrl:String = "\u{8d25}"

/*: "fail to unzip script file" :*/
fileprivate let showAfterData:[Character] = ["f","a","i","l"," ","t","o"," ","u","n","z","i","p"]
fileprivate let noti_ovalPath:String = " scripglass define"

/*: "gift" :*/
fileprivate let show_organizeFormat:String = "gifattach"

/*: "%@%@effect_%ld" :*/
fileprivate let constTowardTitle:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l"]
fileprivate let main_maybeText:String = "law"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let app_showPath:String = "%@%@etrade back retain method"
fileprivate let k_baseballKey:String = "ffect_curve system everyone"
fileprivate let const_immuneKey:[Character] = ["p","T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let app_devoteData:String = "deed theme report variation%@%@ef"
fileprivate let k_footballCostProblemName:String = "both flow market yes out_unzip/"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: CarrierMeasurable?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class EquallyRemainThen: NSObject {
    //: let TalkingGiftAnimatCacheKey  = "CacheGiftEffectCacheKey"
    let TalkingGiftAnimatCacheKey = (String(show_nobodyExFormat.suffix(7)) + String(data_gravityUrl) + String(user_laterConvertValue.suffix(6)))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = EquallyRemainThen()
    ...
    func dataActive(effectMsgModel: NeverthelessTransformable, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if devote(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (kFeeContent))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            bodyPoliticAdmin(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.belowPublicTransport(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (user_storageData.capitalized + "动画\u{6587}件"))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.belowPublicTransport(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func buryModel(fileName: String, model: NeverthelessTransformable) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = originRe(animatMsgModel: model)
    ...
    func doingly() -> String {
        ...
        let subPath = (String(noti_keepPath))
    ...
    func trademarkSchedule(filePath: String) -> String {
    ...
    func cellCenter(model: NeverthelessTransformable) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !devote(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func animat(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((String(kExpectedData) + String(notiCookieScanMessage))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: BeamOfLightItemView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(appLovelyBeyondData.suffix(5)))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: SelfPropelledVehicleThen.self)
        }
        //: return newName
        return newName
    }
...
extension EquallyRemainThen {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func belowPublicTransport(effectMsgModel: NeverthelessTransformable, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = originRe(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((const_heavyTriggerText.replacingOccurrences(of: "pause", with: "o") + String(dataBroadMessage.prefix(4)) + "json"))
        ...
        let dic = NSDictionary.connectCrop(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = CarrierMeasurable()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.plant(dic: dic)
        ...
        className = animat(oldName: className)
    ...
    func devote(model: NeverthelessTransformable) -> Bool {
    ...
    func bodyPoliticAdmin(effectMsgModel: NeverthelessTransformable, errorBlock: @escaping ErrorBlock) {
        ...
        ChatThen.coincide(giftUrl: effectMsgModel.effectUrl, path: outAge(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.click(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func click(effectMsgModel: NeverthelessTransformable, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(notiStickKey.suffix(4))) + "\(mainRequestMsg)" + (String(constMovieTitle.prefix(4))), code: ExtendBasicType.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = outAge(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = layer(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (dataCostId.replacingOccurrences(of: "resistance", with: "ey")) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (user_holdStatusDateId.replacingOccurrences(of: "worker", with: "i")) {
                        // script file
                ...
                printLog(message: (String(const_keepId.prefix(6)) + "to unzip " + kFirstCommitData.replacingOccurrences(of: "stand", with: "i")))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(notiStickKey.suffix(4))) + "\(mainRequestMsg)" + (String(constMovieTitle.prefix(4))), code: ExtendBasicType.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = KeyConsumeChin.rawData(NSData(contentsOfFile: keyFilePath) as Data?, rootCounterval: userMeetingKey)
                    ...
                    let md5 = trademarkSchedule(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(notiStickKey.suffix(4))) + "\(mainRequestMsg)" + (String(constMovieTitle.prefix(4))), code: ExtendBasicType.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (const_denyBringLoopId + user_transformUrl))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(notiStickKey.suffix(4))) + "\(mainRequestMsg)" + (String(constMovieTitle.prefix(4))), code: ExtendBasicType.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = languageModel(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(showAfterData) + String(noti_ovalPath.prefix(6)) + "t file"))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(notiStickKey.suffix(4))) + "\(mainRequestMsg)" + (String(constMovieTitle.prefix(4))), code: ExtendBasicType.UnzipFailed.rawValue))
    ...
    func pictureWindow(path: String) -> Bool {
    ...
    func enrichmentUponType(type: WrongMultiplierTarget) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (show_organizeFormat.replacingOccurrences(of: "attach", with: "t"))
        //: break
        //: default:
        default:
            //: break
    ...
    func outAge(model: NeverthelessTransformable) -> String {
        ...
        let downloadTmpPath = String(format: (String(constTowardTitle) + main_maybeText.replacingOccurrences(of: "law", with: "d")), NSTemporaryDirectory(), enrichmentUponType(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func layer(model: NeverthelessTransformable) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(app_showPath.prefix(5)) + String(k_baseballKey.prefix(6)) + "%ld_unzi" + String(const_immuneKey)), NSTemporaryDirectory(), enrichmentUponType(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func languageModel(model: NeverthelessTransformable) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(app_devoteData.suffix(6)) + "fect_%ld" + String(k_footballCostProblemName.suffix(7))), NSTemporaryDirectory(), enrichmentUponType(type: model.effectType), model.eid)
