...
/*: "num" :*/
fileprivate let app_analysisPath:String = "nuutilize"
...
@objc public enum IdeaMirrorPath: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct WorkerThanksModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func standUp(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct CapabilityHandyJSON: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (app_analysisPath.replacingOccurrences(of: "utilize", with: "m"))
    }
}
