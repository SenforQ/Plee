...
/*: "call_bgm" :*/
fileprivate let const_secretCookieFormat:String = "call_bgmwing soul manage prove both"

/*: "call_response_bgm" :*/
fileprivate let showDismissUrl:[Character] = ["c","a","l","l","_","r","e","s","p"]
fileprivate let app_themCenterContent:String = "icon space to bullet protectiononse_bgm"

/*: "invitePop_call_bgm" :*/
fileprivate let showFindPortFormat:[Character] = ["i","n","v","i","t","e","P","o","p","_","c"]
fileprivate let kFlowName:[Character] = ["a"]
fileprivate let main_withoutUrl:String = "ll_bgmtemp quality market like disk"
...
// __CLOSE_PRINT__
//
//  GraphSoundReactiveCompatible.swift
//  MLProject
//
...
class GraphSoundReactiveCompatible: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = DocumentCountLiteral.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func getableCos(type: DocumentCountLiteral) -> GraphSoundReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = GraphSoundReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.supportSystem()
        //: return player
        return player
    }
...
extension GraphSoundReactiveCompatible {
    //: func play() {
    func quotePlay() {
    ...
    private func flag(notify: Notification) {
    ...
    private func listenerNotify(notify: Notification) {
    ...
    private func supportSystem() {
        ...
        var fileName = (String(const_secretCookieFormat.prefix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(showDismissUrl) + String(app_themCenterContent.suffix(8)))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (String(showFindPortFormat) + String(kFlowName) + String(main_withoutUrl.prefix(6)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = PortraitJackEffectTool.default.topType(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension GraphSoundReactiveCompatible: TXVodPlayListener {
