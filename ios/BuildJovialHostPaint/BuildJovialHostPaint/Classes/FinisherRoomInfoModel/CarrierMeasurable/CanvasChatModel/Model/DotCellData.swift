...
/*: "#BAFBFF" :*/
fileprivate let dataFlexibleFormat:String = "running now#BAFBFF"
...
class DotCellData: NSObject {
    ...
    func operateUs(model: EntryMeasurable) -> EntryMeasurable {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func boxTarget() {
        ...
        let name = tillColor(text: firstStr, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(dataFlexibleFormat.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = tillColor(text: self.model.content, font: UIFont.telescopic(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        receiveContent(attribText: self.model.msgAttribText)
    }
    ...
    func farReason() {
        ...
        let content = tillColor(text: self.model.transContent, font: UIFont.telescopic(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func noteContain() -> NSMutableParagraphStyle {
    ...
    func tillColor(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = noteContain()
        //: return attribute
        return attribute
    }
    ...
    func receiveContent(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = user_changeScaleContent - 36
