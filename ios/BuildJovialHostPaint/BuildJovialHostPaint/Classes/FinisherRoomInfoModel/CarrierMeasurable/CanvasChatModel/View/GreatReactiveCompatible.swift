...
fileprivate let constFavoriteTitle:[UInt8] = [0xec,0xeb,0xec,0xf1,0xad,0xe6,0xea,0xe1,0xe0,0xf7,0xbf,0xac,0xa5,0xed,0xe4,0xf6,0xa5,0xeb,0xea,0xf1,0xa5,0xe7,0xe0,0xe0,0xeb,0xa5,0xec,0xe8,0xf5,0xe9,0xe0,0xe8,0xe0,0xeb,0xf1,0xe0,0xe1]

private func directLittle(makeup num: UInt8) -> UInt8 {
    return num ^ 133
}

/*: "TRTC 视频通话向SDK发送采集的视频帧图片:  :*/
fileprivate let const_restoreData:[Character] = ["T","R","T","C"," ","视","频","通","话","向","S","D","K","发","送","采","\u{96c6}","的","视","频","帧"]
fileprivate let k_solarFormat:[Character] = ["图","\u{7247}",":"," "]

/*: "LIVE 直播向SDK发送采集的视频帧图片:  :*/
fileprivate let show_environmentData:[Character] = ["L","I","V","E"," ","直","播","向","S","D","K","发","送","采","集","\u{7684}","视","频","\u{5e27}","图","片",":"]
fileprivate let showSheContent:[Character] = [" "]
...
class GreatReactiveCompatible: UIView {
    // 直播推流类
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constFavoriteTitle.map{directLittle(makeup: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var captureView: NetView = {
...
extension GreatReactiveCompatible {
    /// 开启本地预览
    /// - Parameters:
    ///   - isSmallWindow: 是否自己在小窗口
    ///   - livePusher: 是否用tx推流
    //: func startPreView(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
    func onMakeupPusher(isSmallWindow: Bool, livePusher: TXLivePush? = nil) {
        ...
        if self.captureView.superview == nil {
            ...
            self.captureView.stCamera.change()
        }
        //: self.captureView.setupVideoConversationPreviewSize(isSmallWindow)
        self.captureView.to(isSmallWindow)
    }
    ...
    func spaceSave() {
        ...
        self.captureView.stCamera.remove()
    ...
    func valueFront(isFront _: Bool) {
        //: guard SenseTime_Use == true else { return }
        guard data_progressName == true else { return }
        //: guard self.captureView.stCamera != nil else {
        guard self.captureView.stCamera != nil else {
...
extension GreatReactiveCompatible: DelegeteWoman {
    // TRTC推流
    //: func stVideoBeautyView(_ beautyView: STFilterView!, didProcessVideoSampleBuffer sampleBuffer: CVPixelBuffer!) {
    func price(_: NetView!, everyPosition sampleBuffer: CVPixelBuffer!) {
        ...
        if trtcCallCount >= logReportInterval {
            ...
            if let image = hunchImage(sampleBuffer) {
                //: UploadLogTool.writeLog(msg: "TRTC 视频通话向SDK发送采集的视频帧图片: \(image.size)")
                GraphicCanvasThen.mnemonics(msg: (String(const_restoreData) + String(k_solarFormat)) + "\(image.size)")
            }
            //: trtcCallCount = 0
            trtcCallCount = 0
        }
    ...
    func sampleFrameBuffer(_: NetView!, fill sampleBuffer: CMSampleBuffer!) {
        //: if livePusher != nil {
        if livePusher != nil {
            ...
            if liveCallCount >= logReportInterval {
                ...
                if let imageBuffer = CMSampleBufferGetImageBuffer(sampleBuffer) {
                    ...
                    if let image = hunchImage(imageBuffer) {
                        //: UploadLogTool.writeLog(msg: "LIVE 直播向SDK发送采集的视频帧图片: \(image.size)")
                        GraphicCanvasThen.mnemonics(msg: (String(show_environmentData) + String(showSheContent)) + "\(image.size)")
                    }
                }
    ...
    func hunchImage(_ imageBuffer: CVImageBuffer) -> UIImage? {
