...
fileprivate let app_cornerKey:[UInt8] = [0xb6,0xb1,0xb6,0xab,0xf7,0xbc,0xb0,0xbb,0xba,0xad,0xe5,0xf6,0xff,0xb7,0xbe,0xac,0xff,0xb1,0xb0,0xab,0xff,0xbd,0xba,0xba,0xb1,0xff,0xb6,0xb2,0xaf,0xb3,0xba,0xb2,0xba,0xb1,0xab,0xba,0xbb]

private func conceptAlbum(disabled num: UInt8) -> UInt8 {
    return num ^ 223
}
...
enum CharacterisationValueConvertible: Int {
    //: case TimeGap = 0
...
@objc protocol FileAllocationTableViewDelegate: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func writeScroll()

    //: @objc optional func endScroll()
    @objc optional func buryOf()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class RemainDataSource: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(flowFire))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_cornerKey.map{conceptAlbum(disabled: $0)}, encoding: .utf8)!)
    }
...
extension RemainDataSource {
    ...
    func familyTimer() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(deed), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func resistanceTimer() {
...
extension RemainDataSource {
    ...
    func inkpad(msgModel: EntryMeasurable?) {
    ...
    func week() {
        ...
        if AllHeight > userSessionStr {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < userSessionStr {
                ...
                self.msgTableView.height = userSessionStr
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = userSessionStr - AllHeight
        ...
        okMakeup(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func okMakeup(animated: Bool) {
...
extension RemainDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func openTogether(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.buryOf?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(noti_replaceMomentValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = EvenThen.animaMutual(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension RemainDataSource {
    ...
    func ditVid() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: user_changeScaleContent, height: userSessionStr)
    }
}
