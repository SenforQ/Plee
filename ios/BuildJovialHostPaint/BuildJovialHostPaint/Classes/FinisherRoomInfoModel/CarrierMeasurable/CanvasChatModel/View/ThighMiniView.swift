...
fileprivate let notiOrientationKey:[UInt8] = [0x20,0x27,0x20,0x3d,0x61,0x2a,0x26,0x2d,0x2c,0x3b,0x73,0x60,0x69,0x21,0x28,0x3a,0x69,0x27,0x26,0x3d,0x69,0x2b,0x2c,0x2c,0x27,0x69,0x20,0x24,0x39,0x25,0x2c,0x24,0x2c,0x27,0x3d,0x2c,0x2d]

/*: "icon_mini_voice_end" :*/
fileprivate let const_motivationTitle:[UInt8] = [0xd2,0xd8,0xd4,0xd5,0xe4,0xd6,0xd2,0xd5,0xd2,0xe4,0xcd,0xd4,0xd2,0xd8,0xde,0xe4,0xde,0xd5,0xdf]

private func publicTransport(gum num: UInt8) -> UInt8 {
    return num ^ 187
}

/*: "icon_mini_voice_calling" :*/
fileprivate let dataContainerContent:String = "ICON"
fileprivate let showInjuryValue:String = "multiplec"
fileprivate let main_receiverMsg:String = "e_caedit examine"

/*: "#F95151" :*/
fileprivate let mainHomeMessage:String = "decision lab crush motivation#F95151"

/*: "#34C759" :*/
fileprivate let const_yellowCaptureValue:String = "note harvest enhance#34C7"
fileprivate let appCoordinatorData:[Character] = ["5","9"]

/*: "End of call" :*/
fileprivate let const_busyId:String = "End of caquick robot should"
fileprivate let main_commercialPath:[Character] = ["l","l"]
...
class ThighMiniView: AmpleThen {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiOrientationKey.map{$0^73}, encoding: .utf8)!)
    }
...
extension ThighMiniView {
    /// 构建音频小窗口
    //: static func buildVoiceMiniView() -> TalkingVoiceMiniView {
    static func costSignal() -> ThighMiniView {
        //: let view = TalkingVoiceMiniView(frame: .zero)
        let view = ThighMiniView(frame: .zero)
    ...
    func talkingTime(_ time: Double) {
...
extension ThighMiniView {
    ...
    private func guide() {
        ...
        simplyStatus(status: .calling)
    }

    /// 刷新视图
    /// - Parameter status: 通话状态
    //: private func refreshVoiceView(status: MiniWindowStatus) {
    private func simplyStatus(status: LapWindowStatus) {
        //: self.currStatus = status
        self.currStatus = status
        //: let iconStr = (status == .end) ? "icon_mini_voice_end":"icon_mini_voice_calling"
        let iconStr = (status == .end) ? String(bytes: const_motivationTitle.map{publicTransport(gum: $0)}, encoding: .utf8)! : (dataContainerContent.lowercased() + "_mini_v" + showInjuryValue.replacingOccurrences(of: "multiple", with: "oi") + String(main_receiverMsg.prefix(4)) + "lling")
        //: self.statusIcon.image = UIImage.BundleImageNamed(name: iconStr)
        self.statusIcon.image = UIImage.adjust(name: iconStr)
        //: self.timeLab.textColor = (status == .end) ? UIColor(hex: "#F95151"):UIColor(hex: "#34C759")
        self.timeLab.textColor = (status == .end) ? UIColor(hex: (String(mainHomeMessage.suffix(7)))) : UIColor(hex: (String(const_yellowCaptureValue.suffix(5)) + String(appCoordinatorData)))
        //: if status == .end {
        if status == .end {
            //: self.timeLab.text = "End of call".localized
            self.timeLab.text = (String(const_busyId.prefix(9)) + String(main_commercialPath)).localized
        }
    }
}
