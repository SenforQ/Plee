...
fileprivate let show_appealId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "UITableViewCell" :*/
fileprivate let dataPureHereConnectionFormat:[Character] = ["U","I","T","a","b","l","e","V","i","e","w"]
fileprivate let main_clickData:[Character] = ["C","e","l","l"]
...
class EvenThen: UITableViewCell {
    //: var msgModel = TalkingVideoCallDanmuModel.init()
    var msgModel = EntryMeasurable()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.msgLabel.snp.makeConstraints { make in
            ...
            make.top.equalTo(noti_replaceMomentValue + 2)
...
//            make.trailing.lessThanOrEqualTo(-32)
        ...
        self.bgLb.snp.makeConstraints { make in
            ...
            make.top.equalTo(noti_replaceMomentValue)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_appealId.reversed(), encoding: .utf8)!)
    }
...
extension EvenThen {
    ...
    class func animaMutual(tableView: UITableView, msg: EntryMeasurable, indexPath _: IndexPath) -> EvenThen {
        ...
        if msg.messageType == 1 {
            ...
            let identifier = "\(ThanksReactiveCompatible.className())_\(msg.messageType)"
            //: tableView.register(TalkingVideoDanmuMsgTextCell.self, forCellReuseIdentifier: identifier)
            tableView.register(ThanksReactiveCompatible.self, forCellReuseIdentifier: identifier)
            //: cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingVideoDanmuMsgTextCell
            cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ThanksReactiveCompatible
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgTextCell(style: .default, reuseIdentifier: identifier)
                cell = ThanksReactiveCompatible(style: .default, reuseIdentifier: identifier)
            }
            //: }else {
        } else {
            //: let identifier = "UITableViewCell"
            let identifier = (String(dataPureHereConnectionFormat) + String(main_clickData))
            //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            var cell = tableView.dequeueReusableCell(withIdentifier: identifier)
            //: if cell == nil {
            if cell == nil {
                //: cell = TalkingVideoDanmuMsgBassCell(style: .default, reuseIdentifier: identifier)
                cell = EvenThen(style: .default, reuseIdentifier: identifier)
            }
            //: return cell as! TalkingVideoDanmuMsgBassCell
            return cell as! EvenThen
        }
