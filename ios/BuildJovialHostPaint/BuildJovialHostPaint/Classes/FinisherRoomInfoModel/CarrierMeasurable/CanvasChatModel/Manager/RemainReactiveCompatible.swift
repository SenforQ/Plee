...
/*: "Speaker On" :*/
fileprivate let main_accuracyValue:String = "floor safety citySpea"
fileprivate let k_wireFormat:[Character] = ["O","n"]

/*: "Speaker Off" :*/
fileprivate let data_yourselfText:String = "psychological straightSpeak"
fileprivate let userPresentationName:[Character] = ["e","r"," ","O","f","f"]

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let mainFirmText:String = "theory orientationTRTC "
fileprivate let const_positionKey:String = "毁. logId\u{ff1a}"

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let dataQuantityPath:[Character] = ["T","R","T","C"," ","\u{97f3}","频","通","话","错","误","."]
fileprivate let app_writerName:String = "acquisition busy rating reason err"

/*: , errMsg: :*/
fileprivate let noti_teamTitle:[Character] = [","," ","e","r","r","M","s","g",":"]

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let appUniformFragmentUrl:[Character] = ["T","R","T","C"," ","音","\u{9891}","通","话","S","D","K","收","\u{5230}"]
fileprivate let data_qualityExtraUrl:String = "design lower duty wait large警告. w"
fileprivate let const_representId:String = "don drag endngCode:"

/*: , warningMsg: :*/
fileprivate let mainProgramAppearancePath:String = "icon bag complaint, wa"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let k_resourceMessage:[Character] = ["T","R","T","C"," ","音"]
fileprivate let constDoingBetterName:String = "频通display进入"
fileprivate let kChildMessage:String = "average remain exceptsult:"

/*: , logId： :*/
fileprivate let mainRelationName:String = "character in midnight style, logId："

/*:  加入了房间" :*/
fileprivate let data_quoteStr:[Character] = [" ","\u{52a0}","\u{5165}","\u{4e86}","房","间"]

/*:  离开了房间" :*/
fileprivate let main_mustPath:[Character] = [" ","离","开","\u{4e86}","房"]
fileprivate let main_stillName:[Character] = ["间"]
...
// __CLOSE_PRINT__
//
//  RemainReactiveCompatible.swift
//  AbroadTalking
//
...
private class MethodChatConfig: NSObject {
...
class RemainReactiveCompatible: NSObject {
    ...
    private var config = MethodChatConfig() // 音频渲染配置类
    //: private var logId = 0
...
extension RemainReactiveCompatible {
    ...
    func pair(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !primary()
    ...
    func backUp(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard primary() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(main_accuracyValue.suffix(4)) + "ker " + String(k_wireFormat)).localized : (String(data_yourselfText.suffix(5)) + String(userPresentationName)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        descriptionDaily(isHandsfree: isHandsfree)
    }
}
...
extension RemainReactiveCompatible {
    ...
    func componentThat() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(transportLine(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(queryionAttention(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(opposition(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(oppositeNotif(notif:)),
    ...
    @objc private func transportLine(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func queryionAttention(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func opposition(notif: Notification) {
    ...
    @objc func oppositeNotif(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            descriptionDaily(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(main_accuracyValue.suffix(4)) + "ker " + String(k_wireFormat)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            descriptionDaily(isHandsfree: self.config.isHandsfree)
...
extension RemainReactiveCompatible {
    ...
    private func descriptionDaily(isHandsfree: Bool) {
    ...
    private func betweenThrough() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        GraphicCanvasThen.mnemonics(msg: (String(mainFirmText.suffix(5)) + "\u{97f3}\u{9891}通话销" + const_positionKey) + "\(logId).")
        //: guard self.config.isFisher else { return }
        guard self.config.isFisher else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            descriptionDaily(isHandsfree: true)
        }
...
extension RemainReactiveCompatible: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GraphicCanvasThen.mnemonics(msg: (String(dataQuantityPath) + String(app_writerName.suffix(4)) + "Code:") + "\(errCode)" + (String(noti_teamTitle)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GraphicCanvasThen.mnemonics(msg: (String(appUniformFragmentUrl) + String(data_qualityExtraUrl.suffix(5)) + "arni" + String(const_representId.suffix(7))) + "\(warningCode)" + (String(mainProgramAppearancePath.suffix(4)) + "rningMsg:") + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        GraphicCanvasThen.mnemonics(msg: (String(k_resourceMessage) + constDoingBetterName.replacingOccurrences(of: "display", with: "话") + "房间. re" + String(kChildMessage.suffix(5))) + "\(result)" + (String(mainRelationName.suffix(8))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (String(data_quoteStr)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(main_mustPath) + String(main_stillName)))
    }
}
