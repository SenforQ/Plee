...
/*: "mf/call/GetUserInfo" :*/
fileprivate let show_whitePath:[Character] = ["m","f","/","c","a","l","l","/","G","e","t","U","s","e"]
fileprivate let mainGuideMessage:[Character] = ["r","I","n","f","o"]

/*: "uid" :*/
fileprivate let mainErrTitle:[UInt8] = [0x64,0x69,0x75]

/*: "TRTC 随机视频匹配中." :*/
fileprivate let kWriterData:String = "in enterprise alongside explain relatedTRTC "
fileprivate let const_backgroundFormat:String = "匹配中."

/*: "TRTC  :*/
fileprivate let showClickInstallFormat:String = "tag clockTRTC "

/*: 视频通话. logId: :*/
fileprivate let showTentPortText:String = "视\u{9891}通\u{8bdd}."
fileprivate let main_sinceStatPath:String = ":"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let appSuperiorPrimeContent:String = "TRTC 视decide extra relation carve"
fileprivate let mainShotMsg:String = "situation beneath firm broad毁. logId："

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let app_wedFormat:String = "TRTC union purse cur straight cos"
fileprivate let dataBitUrl:String = "误. eoval direction going assist form"
fileprivate let main_goldMsg:String = "broadcast equallye:"

/*: , errMsg: :*/
fileprivate let k_positiveMsg:String = "found action sum beyond chemical, errM"
fileprivate let const_naturalGenderValue:[Character] = ["s","g",":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let noti_enterprisePath:String = "equityTC"
fileprivate let noti_infoValue:String = "support component whole net processorSDK收"
fileprivate let const_accomplishData:String = "aoptionnin"
fileprivate let app_darkPath:String = "self glasse:"

/*: , warningMsg: :*/
fileprivate let k_factorPath:String = ", wgive week mention"
fileprivate let mainMuttValue:[Character] = ["a","r","n","i","n","g","M","s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let dataSourceStanceName:String = "TRTC factor theoretical tender"
fileprivate let kStretchStr:[Character] = ["房","间","."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let k_towardStr:String = ", logId\u{ff1a}"

/*:  加入了房间" :*/
fileprivate let app_socialPath:[Character] = [" ","加","\u{5165}","了","房","间"]

/*:  离开了房间" :*/
fileprivate let mainHeavySceneMsg:[Character] = [" ","离","开","了","房","间"]
...
// __CLOSE_PRINT__
//
//  PortraitExcitementReactiveCompatible.swift
//  AbroadTalking
//
...
enum GraphicExcitementStrideable: Int {
    //: case CallEnd     = 100
...
enum RoundTelevisionCameraStatus {
    //: case Normal
    case Normal // 还未完成初始化
    //: case Open
    case Open // 开启
...
class ComponentThen: NSObject {
    ...
    var remoteCameraStatus = RoundTelevisionCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class PortraitExcitementReactiveCompatible: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: CanvasChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = ComponentThen() // 视频渲染配置类
    ...
    private var trtcView: GreatReactiveCompatible? // 预览view（使用商汤美颜）
...
extension PortraitExcitementReactiveCompatible {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func paraCompletion(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = ManhoodRequestModel()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(show_whitePath) + String(mainGuideMessage))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: mainErrTitle.reversed(), encoding: .utf8)!: uid]
...
extension PortraitExcitementReactiveCompatible {
    ...
    func beyondResponse(_ frontCamera: Bool) {
        ...
        self.frontHalf(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func quote(_ openCamera: Bool) {
        ...
        self.alright(isOpen: openCamera)
    }
    ...
    func writeLanguage() {
        ...
        DispatchQueue.main.async {
            ...
            self.ta(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
    ...
    func rateOfPublish() {
        // 如果没有预览先开启预览
        //: self.config.isFisher = true
        self.config.isFisher = true
        //: if self.config.isPreviewNow == false {
        if self.config.isPreviewNow == false {
            //: self.startPreview()
            self.tetrasodiumPyrophosphate()
        }
...
extension PortraitExcitementReactiveCompatible {
    ...
    private func frontHalf(front: Bool) {
        ...
        self.trtcView?.valueFront(isFront: front)
    }

    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func alright(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard data_progressName == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.opend(isOpen: isOpen)
...
extension PortraitExcitementReactiveCompatible {
    ...
    func handle() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(deliverTheGoods(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(captureProperty(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(charmThen(notif:)),
    ...
    @objc private func deliverTheGoods(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func nigh(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func captureProperty(notif: Notification) {
        ...
        DispatchQueue.main.async {
            ...
            if interruptStatus == AVAudioSession.InterruptionType.began {
                ...
                self.deliverTheGoods(notif: nil)
    ...
    @objc func charmThen(notif: Notification) {
...
extension PortraitExcitementReactiveCompatible {
    ...
    func tetrasodiumPyrophosphate(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            GraphicCanvasThen.mnemonics(msg: (String(kWriterData.suffix(5)) + "随机视频" + const_backgroundFormat.capitalized))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            GraphicCanvasThen.mnemonics(msg: (String(showClickInstallFormat.suffix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (showTentPortText + " logId" + main_sinceStatPath.capitalized) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            ta(smallWindow: false)
    ...
    func ta(smallWindow: Bool) {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = GreatReactiveCompatible()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.onMakeupPusher(isSmallWindow: smallWindow)
        //: if smallWindow {
    ...
    private func pure() {
        ...
        params.sdkAppId = dataInviteId
        //: params.roomId = UInt32(roomId)
        params.roomId = UInt32(roomId)
        //: params.userId = RealmReactiveCompatible.share.loginUserMode.userID
        params.userId = RealmReactiveCompatible.share.loginUserMode.userID
        //: params.userSig = RealmReactiveCompatible.share.appUserConfigMode.uSigTrtc
        params.userSig = RealmReactiveCompatible.share.appUserConfigMode.uSigTrtc
    ...
    private func imperil() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        GraphicCanvasThen.mnemonics(msg: (String(appSuperiorPrimeContent.prefix(6)) + "频通\u{8bdd}销" + String(mainShotMsg.suffix(9))) + "\(infoModel?.logId ?? 0).")
...
extension PortraitExcitementReactiveCompatible: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GraphicCanvasThen.mnemonics(msg: (String(app_wedFormat.prefix(5)) + "视频通\u{8bdd}错" + String(dataBitUrl.prefix(4)) + "rrCod" + String(main_goldMsg.suffix(2))) + "\(errCode)" + (String(k_positiveMsg.suffix(6)) + String(const_naturalGenderValue)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.bringHome(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        GraphicCanvasThen.mnemonics(msg: (noti_enterprisePath.replacingOccurrences(of: "equity", with: "TR") + " 视频通话" + String(noti_infoValue.suffix(4)) + "到警告. w" + const_accomplishData.replacingOccurrences(of: "option", with: "r") + "gCod" + String(app_darkPath.suffix(2))) + "\(warningCode)" + (String(k_factorPath.prefix(3)) + String(mainMuttValue)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: const_depthTitle, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        GraphicCanvasThen.mnemonics(msg: (String(dataSourceStanceName.prefix(5)) + "视频通话进入" + String(kStretchStr)) + "\(result)" + (k_towardStr) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            ...
            ManagerCommentThen.shared.ample()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != RealmReactiveCompatible.share.loginUserMode.userID else { return }
        guard userId != RealmReactiveCompatible.share.loginUserMode.userID else { return }
