...
fileprivate let appSenseTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
@objc protocol LiteralObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func originScroll()

    //: @objc optional func endScroll()
    @objc optional func taPhoto()
}

//: class TalkingDanmuMsgListTableView: UIView {
class WalkViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appSenseTitle.reversed(), encoding: .utf8)!)
    }
...
extension WalkViewDelegate {
    //: func startTimer() {
    func summer() {
        //: stopTimer()
        calledActualContrast()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(quitAcrossEvent), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func calledActualContrast() {
...
extension WalkViewDelegate {
    ...
    func meReason(msgModel: AcrossHandyJSON?) {
    ...
    func narrowMemory() {
        ...
        if AllHeight > show_numberPath {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < show_numberPath {
                ...
                self.msgTableView.height = show_numberPath
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = show_numberPath - AllHeight
        ...
        haveBegin(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func haveBegin(animated: Bool) {
    ...
    func comparePending(inPending: Bool) {
...
extension WalkViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(app_infoPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ForbidReactiveCompatible.pinion(tableView: tableView, msg: msgModel, indexPath: indexPath)
