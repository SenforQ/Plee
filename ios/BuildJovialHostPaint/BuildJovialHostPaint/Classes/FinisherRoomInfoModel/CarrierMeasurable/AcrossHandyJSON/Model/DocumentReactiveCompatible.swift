...
/*: "MF:LiveChatWelMsg" :*/
fileprivate let kInformStr:String = "trail talk a editorMF:Liv"
fileprivate let dataConcentrationName:String = "surgery chart invite channelWelM"
fileprivate let userGradeKey:String = "SG"

/*: "MF:LiveChatPrizeMsg" :*/
fileprivate let data_countAreaStr:String = "MF:Liveidentity revenue spread"
fileprivate let dataIssueId:String = "property"
fileprivate let appMakerUrl:[Character] = ["h","a","t","P","r","i","z","e","M","s","g"]

/*: "MF:LiveChatAttentionMsg" :*/
fileprivate let show_processMessage:String = "deadline exhibit event within awayMF:LiveCh"
fileprivate let data_remoteEnoughMessage:String = "atAtremind stuff content display"
fileprivate let k_validName:String = "onMsgtot enable want"

/*: "#BAFBFF" :*/
fileprivate let showTextMessage:String = "animal press#BAFBFF"

/*: "uid" :*/
fileprivate let dataFollowNapTitle:String = "coreid"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let show_statusPauseValue:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]"]
fileprivate let k_cutStr:[Character] = ["+","?","<","/","a","t",">"]

/*: "Sent " :*/
fileprivate let appRecognizeMessage:String = "region partner component branch egoSent "

/*: "#FFA4CF" :*/
fileprivate let appMoreStr:String = "platform eastern super reference price#FFA4CF"

/*: "Here comes " :*/
fileprivate let userSunFlatUrl:[Character] = ["H","e","r","e"," ","c","o","m","e","s"," "]

/*: "#FFE386" :*/
fileprivate let show_cornerStr:[Character] = ["#","F","F","E","3","8","6"]

/*: "followed the anchor" :*/
fileprivate let notiHaveMsg:[Character] = ["f","o","l"]
fileprivate let appAverageMessage:String = "lowed trail drama support attach dot"
fileprivate let dataLowValue:String = "choscreen"

/*: "level_ :*/
fileprivate let dataClothingId:[Character] = ["l","e","v","e","l","_"]

/*: "icon_lounge" :*/
fileprivate let mainDevilPath:String = "icon_trust equity snap name don"
...
// __CLOSE_PRINT__
//
//  DocumentReactiveCompatible.swift
//  AbroadTalking
//
...
class DocumentReactiveCompatible: NSObject {
    ...
    func mergeModel(model: AcrossHandyJSON) -> AcrossHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息,礼物消息
        ...
        } else if self.model.MsgExtension == (String(kInformStr.suffix(6)) + "eChat" + String(dataConcentrationName.suffix(4)) + userGradeKey.lowercased()) {
            //: caculateWelMsg()
            maxilla()
            /// 中奖消息
            //: } else if self.model.MsgExtension == "MF:LiveChatPrizeMsg" {
        } else if self.model.MsgExtension == (String(data_countAreaStr.prefix(7)) + dataIssueId.replacingOccurrences(of: "property", with: "C") + String(appMakerUrl)) {
        ...
        } else if self.model.MsgExtension == (String(show_processMessage.suffix(9)) + String(data_remoteEnoughMessage.prefix(4)) + "tenti" + String(k_validName.prefix(5))) {
            //: caculAttionMsg()
            attributeMsg()
        }
        //: return self.model
    ...
    func questionable() {
        ...
        let nameAttr = toolBusy(text: firstStr, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(showTextMessage.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        ...
        if self.model.msgInfo?.messageType == 1 {
            ...
            let content = toolBusy(text: self.model.msgInfo?.content ?? "", font: UIFont.telescopic(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let atUid = jumps?[(dataFollowNapTitle.replacingOccurrences(of: "core", with: "u"))] as? Int ?? 0
            //: let text = self.model.msgInfo?.content ?? ""
            let text = self.model.msgInfo?.content ?? ""
            //: let regex = try! NSRegularExpression(pattern: "<at>@[\\S\\s]+?</at>", options: [])
            let regex = try! NSRegularExpression(pattern: (String(show_statusPauseValue) + String(k_cutStr)), options: [])
            ...
            if matchRange?.length ?? 0 > 9 {
                ...
                let subStr = String.cooperative(str: text, from: (matchRange?.location ?? 0) + 4, to: (matchRange?.length ?? 0) - 5)
                //: let contentStr = String.substringByUnicodeIndex(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count )
                let contentStr = String.cooperative(str: text, from: (matchRange?.length ?? 0), to: text.utf16.count)
                ...
                let atName = toolBusy(text: subStr, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(showTextMessage.suffix(7))))!, isTap: true, atUid: "\(atUid)")
                ...
                let content = toolBusy(text: contentStr, font: UIFont.telescopic(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            let sentAttr = toolBusy(text: (String(appRecognizeMessage.suffix(5))).localized, font: UIFont.telescopic(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
            ...
            if let toUser = self.model.toUser { // 送礼接收人
                ...
                let nickNameAttr = toolBusy(text: "\(toUser.nickname) ", font: UIFont.shrinkSize(type: .Medium, fontSize: 14), color: UIColor(hex: (String(appMoreStr.suffix(7))))!, isTap: true, atUid: String(toUser.uid))
                //: nameAttr.append(nickNameAttr)
                nameAttr.append(nickNameAttr)
            }
            //: let giftnameStr = changeGiftName(giftmodel: self.model.gift!)
            let giftnameStr = relatedness(giftmodel: self.model.gift!)
            ...
            let giftAttr = toolBusy(text: giftnameStr, font: UIFont.shrinkSize(type: .Medium, fontSize: 14), color: UIColor(hex: (String(appMoreStr.suffix(7))))!, isTap: false, atUid: nil)
            ...
            let imag = self.model.gift?.giftImg?.numbero(with: size)
            ...
            let giftIcon = NSMutableAttributedString.yy_attachmentString(withContent: imag, contentMode: .center, attachmentSize: size, alignTo: UIFont.telescopic(fontSize: 14), alignment: .center)
            //: giftIcon.yy_paragraphStyle = paragraphStyle()
            giftIcon.yy_paragraphStyle = distant()
            ...
            let numAttr = self.toolBusy(text: "*\(self.model.gift?.num ?? 0)", font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(appMoreStr.suffix(7))))!, isTap: false, atUid: nil)
        ...
        passJudgmentProperty(attribText: self.model.msgAttribText)
    }

    /// 公告富文本
    //: func showAnnouncementMsg(model: TalkingLiveRoomDanmuModel) -> TalkingLiveRoomDanmuModel {
    func followInfo(model: AcrossHandyJSON) -> AcrossHandyJSON {
        //: self.model = model
        self.model = model
        // 内容
        //: let paraStyle = paragraphStyle()
        ...
        let content = toolBusy(text: self.model.msgInfo?.content ?? "", font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(showTextMessage.suffix(7))))!, isTap: false, atUid: nil)
    ...
    func maxilla() {
        ...
        let strnor = (String(userSunFlatUrl)).localized
        ...
        let content = toolBusy(text: strnor, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(show_cornerStr)))!, isTap: false, atUid: nil)
        ...
        let attachText = toolBusy(text: strc, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(showTextMessage.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func gum() {
        ...
        let content = toolBusy(text: firstStr, font: UIFont.telescopic(fontSize: 14), color: UIColor.white, isTap: true, atUid: String(self.model.user?.uid ?? 0))
        //: textView.append(content)
        textView.append(content)
        //: self.model.msgAttribText = textView
        self.model.msgAttribText = textView
    ...
    func attributeMsg() {
        ...
        let strnor = (String(notiHaveMsg) + String(appAverageMessage.prefix(6)) + "the an" + dataLowValue.replacingOccurrences(of: "screen", with: "r")).localized
        //: let strc =  "\(self.model.user?.nickname ?? "") "
        let strc = "\(self.model.user?.nickname ?? "") "
        ...
        let content = toolBusy(text: strnor, font: UIFont.telescopic(fontSize: 14), color: UIColor.white, isTap: false, atUid: nil)
        ...
        let nameText = toolBusy(text: strc, font: UIFont.telescopic(fontSize: 14), color: UIColor(hex: (String(showTextMessage.suffix(7))))!, isTap: true, atUid: String(self.model.user?.uid ?? 0))
    ...
    func distant() -> NSMutableParagraphStyle {
    ...
    func toolBusy(text: String, font: UIFont, color: UIColor, isTap: Bool, atUid: String?) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = distant()
        ...
        if isTap == true {
            ...
            attribute.yy_setTextHighlight(range, color: color, backgroundColor: .clear, userInfo: nil) { _, _, _, _ in
                ...
                /// 不能@自己
                //: if atUid != RealmReactiveCompatible.share.loginUserMode.userID {
                if atUid != RealmReactiveCompatible.share.loginUserMode.userID {
    ...
    func passJudgmentProperty(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = MsgTableViewWidth - 16
        let maxWidth = k_groupMaleData - 16
    ...
    private func relatedness(giftmodel: WorkerThanksModelType) -> String {
...
extension DocumentReactiveCompatible {
    /// 获取弹幕富文本图片
    //: func getImagesAttributedString() -> NSMutableAttributedString {
    func patent() -> NSMutableAttributedString {
        ...
        levelImgV.image = UIImage.adjust(name: (String(dataClothingId)) + "\(self.model.user?.level ?? 1)")
        //: let levelAttr = getAttachText(imageView: levelImgV)
        let levelAttr = opposite(imageView: levelImgV)
        ...
        if self.model.user?.loungePlus == true {
            ...
            vipImgV.image = UIImage.adjust(name: (String(mainDevilPath.prefix(5)) + "lounge"))
            //: let vipAttr = getAttachText(imageView: vipImgV)
            let vipAttr = opposite(imageView: vipImgV)
    ...
    func opposite(imageView: UIImageView) -> NSMutableAttributedString {
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView.image, contentMode: .scaleToFill, attachmentSize: imageView.size, alignTo: UIFont.ditMention(fontSize: 12), alignment: .center)
        //: attachText.yy_paragraphStyle = paragraphStyle()
        attachText.yy_paragraphStyle = distant()
        //: return attachText
        return attachText
    }
}
