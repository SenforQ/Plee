...
fileprivate let notiGoldUrl:[UInt8] = [0x7a,0x7d,0x7a,0x67,0x3b,0x70,0x7c,0x77,0x76,0x61,0x29,0x3a,0x33,0x7b,0x72,0x60,0x33,0x7d,0x7c,0x67,0x33,0x71,0x76,0x76,0x7d,0x33,0x7a,0x7e,0x63,0x7f,0x76,0x7e,0x76,0x7d,0x67,0x76,0x77]

private func handDifference(near num: UInt8) -> UInt8 {
    return num ^ 19
}

/*: "icon_coinxxx" :*/
fileprivate let const_completeStr:String = "icoalongside"
fileprivate let dataGravityIgnoreMsg:String = "nagreeagreeagree"

/*: "icon_gift_num_bg" :*/
fileprivate let show_titleMessage:[Character] = ["i","c","o","n","_"]
fileprivate let showDatabaseFormat:[Character] = ["g","i","f","t","_","n","u","m","_","b","g"]
...
class DotGiftCell: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = LimitedModelType()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiGoldUrl.map{handDifference(near: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.shrinkSize(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: PostureTalkingButton = {
        //: let btn = TalkingButton.init()
        let btn = PostureTalkingButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.shrinkSize(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.adjust(name: (const_completeStr.replacingOccurrences(of: "alongside", with: "n") + "_coi" + dataGravityIgnoreMsg.replacingOccurrences(of: "agree", with: "x"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.adjust(name: (String(show_titleMessage) + String(showDatabaseFormat)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.shrinkSize(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension DotGiftCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func inmateQueryed(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.labIndicator().cgColor
    ...
    func frenchTelephone(currenmodel: LimitedModelType, giftType: ComponentColumnConvertible) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.noneAdvancedFinish(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.noneAdvancedFinish(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.shrinkSize(type: .Regular, fontSize: 11),
...
extension DotGiftCell {
    ...
    func belowRecent() {
        ...
        bgView.backgroundColor = UIColor.attachColor()
