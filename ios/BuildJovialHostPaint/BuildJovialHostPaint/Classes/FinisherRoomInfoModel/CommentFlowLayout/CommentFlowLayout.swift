...
class CommentFlowLayout: UICollectionViewFlowLayout {
    ...
    var sectionArr = [ComponentColumnConvertible]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(kMeMsg/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(dataModeBlockStr-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == ComponentColumnConvertible.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == ComponentColumnConvertible.Bags.rawValue {
        ...
        let itemWidth = Int(dataModeBlockStr-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(dataModeBlockStr-10) + pageCol * itemWidth
