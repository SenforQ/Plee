...
fileprivate let show_yaHugeMsg:String = "设置普通状\u{6001}"
fileprivate let show_harassmentTitle:String = "色\u{65f6} 请"
fileprivate let kRootPath:String = "使\u{7528}R"
fileprivate let user_groupFormat:String = "visitor ear pressGB空间的颜色值"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let user_boyMsg:String = "\u{8bbe}置选中状态"
fileprivate let data_vehicleFormat:[Character] = ["用","R","G","B"]
fileprivate let noti_learnMsg:String = "\u{7a7a}间\u{7684}颜色值"

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiFailureText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "设置的下标不合法!!" :*/
fileprivate let dataPlayerFormat:String = "设置的下标"

/*: "Bags" :*/
fileprivate let show_fileKey:String = "Bagsgreet perspective cord"
...
open class PowderSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: ToeSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.match(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((show_yaHugeMsg + "的文字\u{989c}" + show_harassmentTitle + kRootPath + String(user_groupFormat.suffix(8))))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.match(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((user_boyMsg + "的文字颜\u{8272}时 \u{8bf7}使" + String(data_vehicleFormat) + noti_learnMsg))
        }
    ...
    fileprivate func match(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: ToeSegmentStyle, titles: [String]) {
    ...
    @objc func capacityClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? CoverLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiFailureText.reversed(), encoding: .utf8)!)
    }
}
...
public extension PowderSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func count(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (dataPlayerFormat.capitalized + "\u{4e0d}合法!!"))
    ...
    func winkerMid(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
//: extension ScrollSegmentView {
extension PowderSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func fireBold() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = CoverLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.capacityClick(_:)))
            ...
            if title == (String(show_fileKey.prefix(4))).localized {
    ...
    fileprivate func recognizeConfirm() {
        ...
        if let firstLabel = labelsArray[0] as? CoverLabel {
    ...
    public func local(_ currentIndex: Int) {
