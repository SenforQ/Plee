...
/*: "App" :*/
fileprivate let appBlueName:String = "Appstandard tot"

/*: "blockList_ :*/
fileprivate let userWorkValue:[Character] = ["b","l","o","c","k","L","i","s","t"]
fileprivate let notiToleranceMessage:[Character] = ["_"]
...
// __CLOSE_PRINT__
//
//  OptionReactiveCompatible.swift
//  AbroadTalking
//
...
class OptionReactiveCompatible: NSObject {
    //: var blockList: [String]?
    var blockList: [String]? // 拉黑列表

    // singleton
    //: static let shared = TalkingBlockListManager()
    static let shared = OptionReactiveCompatible()
    ...
    private func acquireJawSunna() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(appBlueName.prefix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(userWorkValue) + String(notiToleranceMessage)) + "\(uid)")
        //: return filePath
        return filePath
    }
}
