...
/*: "uid" :*/
fileprivate let showWeightPath:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let showValueOrangeRequiredName:String = "onapplicationne"

/*: "isNewUser" :*/
fileprivate let k_acquireSpanMsg:[Character] = ["i","s","N","e","w","U","s"]
fileprivate let kMergeStr:String = "ejump"

/*: "isTPAuth" :*/
fileprivate let notiHowValue:String = "isTPfee pending"

/*: "totalIntimate" :*/
fileprivate let app_currencyKey:String = "ficeta"
fileprivate let k_strokeTitle:[Character] = ["l","I","n","t","i","m","a","t","e"]

/*: "userStatus" :*/
fileprivate let kInstallData:[Character] = ["u","s","e","r","S","t","a","t","u","s"]

/*: "intimate" :*/
fileprivate let main_environmentStr:String = "intiroute"
fileprivate let k_simultaneouslyTitle:[Character] = ["a","t","e"]
...
class PerceptListManager {
    ...
    private var moreMsgModel: AccessConversationModel = //: return AccessConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .ptolemaicSystem(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func tagOn() {
        //: self.officialArr = DepictionViewManager.shared.getOfficialList()
        self.officialArr = DepictionViewManager.shared.box()
        //: self.topArr = DepictionViewManager.shared.topConvList
        self.topArr = DepictionViewManager.shared.topConvList
        //: self.normalArr = DepictionViewManager.shared.norConvList
        self.normalArr = DepictionViewManager.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        aboveInmate()
    ...
    func objectBridge(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: DepictionViewManager.shared.getChatRoomData { succeed in
        DepictionViewManager.shared.reply { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = DepictionViewManager.shared.chatRoomList
                self.chatRoomArr = DepictionViewManager.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func marginOfError(indexPath: IndexPath) -> AccessConversationModel? {
    ...
    func quoteModel(model: AccessConversationModel?) -> AccessConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: CurrencyModelType? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: CurrencyModelType? = DotInfoManager.magnitudery(targetId: targetId)
    ...
    func adoptiveParent(conversationList: [AccessConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        DotInfoManager.message(with: tempArr) { _, error in
    ...
    func conversationPremium(conversationModel: AccessConversationModel) {
    ...
    func toALesserExtent(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: DepictionViewManager.shared.func__getConversationList { arr, leftMore in
        DepictionViewManager.shared.singleName { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [AccessConversationModel] = //: return Array<AccessConversationModel>()
        .init()
    //: }()
}
...
extension PerceptListManager {
    ...
    func aboveInmate() {
        //: self.moreMsgArr = DepictionViewManager.shared.moreMsgList
        self.moreMsgArr = DepictionViewManager.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func nameTag(indexPath: IndexPath) -> AccessConversationModel? {
    ...
    func pollex() -> AccessConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        aboveInmate()
        ...
        moreMsgModel.unreadCount = messageGet()
...
extension PerceptListManager {
    ...
    func acceptContent(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        DotInfoManager.linkPair(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(showWeightPath))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(showValueOrangeRequiredName.replacingOccurrences(of: "application", with: "li") + "Status")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(k_acquireSpanMsg) + kMergeStr.replacingOccurrences(of: "jump", with: "r"))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(notiHowValue.prefix(4)) + "Auth")].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(app_currencyKey.replacingOccurrences(of: "fice", with: "to") + String(k_strokeTitle))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(kInstallData))].int ?? 1
                    ...
                    self.secondSubmit(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func stepparent(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(showWeightPath))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(showValueOrangeRequiredName.replacingOccurrences(of: "application", with: "li") + "Status")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(k_acquireSpanMsg) + kMergeStr.replacingOccurrences(of: "jump", with: "r"))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(kInstallData))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(kInstallData))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension PerceptListManager {
    ...
    func positiveData() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in DepictionViewManager.shared.norConvList {
        for model in DepictionViewManager.shared.norConvList {
            //: if model.totalIntimate >= RealmReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= RealmReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.quoteModel(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in DepictionViewManager.shared.topConvList {
        for model in DepictionViewManager.shared.topConvList {
            //: if model.totalIntimate >= RealmReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= RealmReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func blackAnonymous(convModel: AccessConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func magnitudeimate(indexPath: IndexPath) -> AccessConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func secondSubmit(uid: String, intimate: Int) {
        ...
        DotInfoManager.enablePull(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func environment(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(showWeightPath))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(main_environmentStr.replacingOccurrences(of: "route", with: "m") + String(k_simultaneouslyTitle))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
