...
fileprivate let show_herPath:[UInt8] = [0x87,0x8c,0x87,0x92,0x46,0x81,0x8d,0x82,0x83,0x90,0x58,0x47,0x3e,0x86,0x7f,0x91,0x3e,0x8c,0x8d,0x92,0x3e,0x80,0x83,0x83,0x8c,0x3e,0x87,0x8b,0x8e,0x8a,0x83,0x8b,0x83,0x8c,0x92,0x83,0x82]

fileprivate func amStretch(length num: UInt8) -> UInt8 {
...
/*: "PositionThen" :*/
fileprivate let data_richFormat:[Character] = ["D","r","o","p","M","e","m"]
fileprivate let main_judgeMsg:[Character] = ["b","e","r","M","e"]
fileprivate let showInputMsg:String = "detect shapenuCell"

/*: "roomId" :*/
fileprivate let data_answerValue:[Character] = ["r","o","o","m","I","d"]

/*: "type" :*/
fileprivate let kObserverValue:[Character] = ["t","y","p","e"]

/*: "page" :*/
fileprivate let constPossibleValue:String = "patabe"

/*: "icon_chats_pcr" :*/
fileprivate let noti_appealMessage:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let constPieceName:[Character] = ["t"]
fileprivate let main_birthdayChartIValue:String = "s_pcrnot center audience"

/*: "All Numbers" :*/
fileprivate let showCapacityStr:String = "All Numdirect guest"
fileprivate let noti_fragmentData:[Character] = ["b","e","r","s"]

/*: "all" :*/
fileprivate let userShadowStr:String = "aresumeresume"

/*: "Recent Gifts" :*/
fileprivate let const_shadeName:String = "generate pop indicator sectionRecent"
fileprivate let user_openingMsg:String = " Giftslimit thanks regular life"
...
protocol PositionViewDelegate {
    ...
    func shareModel(model: EquallyLimitedHypothesisHandyJSON)
}

//: class DropDownMemberMenuView: UIView {
class RoundDataSource: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: PositionViewDelegate?
    ...
    public var triangleHeight = Config.ScoreGravityMulti.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.ScoreGravityMulti.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.addPosition()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.worldView()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = RoundDataSource(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = CurrencyDefineReactiveCompatible.getWindow()
        let window = CurrencyDefineReactiveCompatible.stripElement()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_herPath.map{amStretch(length: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func addPosition() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func worldView() -> UITableView {
    ...
    private func followUi() {
        ...
        mTable.register(PositionThen.self, forCellReuseIdentifier: (String(data_richFormat) + String(main_judgeMsg) + String(showInputMsg.suffix(6))))
    ...
    func narrowData() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(data_answerValue))] = roomID
        //: dict["type"] = "3"
        dict[(String(kObserverValue))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(constPossibleValue.replacingOccurrences(of: "tab", with: "g"))] = String(pageIndex)
        ...
        WrittenRecordReactiveCompatible.share.plusBy(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = EquallyLimitedHypothesisHandyJSON()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(noti_appealMessage) + String(constPieceName) + String(main_birthdayChartIValue.prefix(5)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(showCapacityStr.prefix(7)) + String(noti_fragmentData)).localized
                //: model.uid = "all"
                model.uid = (userShadowStr.replacingOccurrences(of: "resume", with: "l"))
...
extension RoundDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(data_richFormat) + String(main_judgeMsg) + String(showInputMsg.suffix(6))), for: indexPath) as! PositionThen
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! EquallyLimitedHypothesisHandyJSON
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! EquallyLimitedHypothesisHandyJSON
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.brightGreet(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.shareModel(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: dataModeBlockStr, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.smallColor()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .shrinkSize(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .resumeName()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(const_shadeName.suffix(6)) + String(user_openingMsg.prefix(6))).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(showCapacityStr.prefix(7)) + String(noti_fragmentData)).localized
        }
