...
fileprivate let show_futureValue:[UInt8] = [0x98,0x9f,0x98,0x85,0xd9,0x92,0x9e,0x95,0x94,0x83,0xcb,0xd8,0xd1,0x99,0x90,0x82,0xd1,0x9f,0x9e,0x85,0xd1,0x93,0x94,0x94,0x9f,0xd1,0x98,0x9c,0x81,0x9d,0x94,0x9c,0x94,0x9f,0x85,0x94,0x95]

private func carryEntry(when num: UInt8) -> UInt8 {
    return num ^ 241
}

/*: "#30D00B" :*/
fileprivate let userOutputId:String = "#30D00Bdeliver make trace"

/*: "icon_receivedcalls" :*/
fileprivate let appIssueId:String = "icon_by pad"
fileprivate let dataCarrierUrl:String = "eregional"
fileprivate let showCountroValue:[Character] = ["l","s"]

/*: "Received calls" :*/
fileprivate let kCombineMessage:[Character] = ["R","e","c","e","i","v","e"]
fileprivate let data_rawHoweverPutTitle:[Character] = ["d"," ","c","a","l","l","s"]

/*: "icon_dialedcalls" :*/
fileprivate let showPackKey:String = "ion takeicon_"
fileprivate let user_smartPath:[Character] = ["c","a","l","l","s"]

/*: "Dialed calls" :*/
fileprivate let notiRefId:[Character] = ["D","i","a","l","e"]
fileprivate let show_haveMessage:String = "d callsbig assessment speaker fire"
...
class PledgeTableCell: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = LoopHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_futureValue.map{carryEntry(when: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(sourceMove), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .shrinkSize(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .resumeName()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(userOutputId.prefix(7))))
...
extension PledgeTableCell {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func obnubilate(model: LoopHandyJSON) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.resolutionAlbum(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.adjust(name: (String(appIssueId.prefix(5)) + "receiv" + dataCarrierUrl.replacingOccurrences(of: "region", with: "dc") + String(showCountroValue)))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(kCombineMessage) + String(data_rawHoweverPutTitle)).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.adjust(name: (String(showPackKey.suffix(5)) + "dialed" + String(user_smartPath)))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(notiRefId) + String(show_haveMessage.prefix(7))).localized
        }
