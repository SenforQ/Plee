...
class ManagerThen: NSObject {
    ...
    //: var windownsPostion: CGPoint = CGPoint(x: ScreenWidth-4, y: StatusBarNavigationBarHeight+373)   
    var windownsPostion: CGPoint = CGPoint(x: dataModeBlockStr-4, y: constCornerDetailUrl-const_hundredId-66-38)   // 窗口默认位置
...
extension ManagerThen {
    ...
    func conjureView() {
        ...
        let windowView = FoolishnessThen()
        //: let keywindow = CurrencyDefineReactiveCompatible.getWindow()
        let keywindow = CurrencyDefineReactiveCompatible.stripElement()
