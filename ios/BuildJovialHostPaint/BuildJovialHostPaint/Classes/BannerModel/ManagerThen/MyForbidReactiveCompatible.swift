...
fileprivate let userOpMessage:[UInt8] = [0x0,0x7,0x0,0x1d,0x41,0xa,0x6,0xd,0xc,0x1b,0x53,0x40,0x49,0x1,0x8,0x1a,0x49,0x7,0x6,0x1d,0x49,0xb,0xc,0xc,0x7,0x49,0x0,0x4,0x19,0x5,0xc,0x4,0xc,0x7,0x1d,0xc,0xd]

private func rollUp(calculation num: UInt8) -> UInt8 {
    return num ^ 105
}
...
// __CLOSE_PRINT__
//
//  MyForbidReactiveCompatible.swift
//  AbroadTalking
//
...
class MyForbidReactiveCompatible: UIView {
//    var tapGestureBlock: (() -> Void)?   // 点击手势Block
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userOpMessage.map{rollUp(calculation: $0)}, encoding: .utf8)!)
    }
...
extension MyForbidReactiveCompatible {
    /// 添加点击手势
//    @discardableResult
...
//    }
    ...
    func andShrink() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(fryingPanPlace(sender:)))
...
//        pan.shouldBeRequiredToFail(by: tap)
    ...
    @objc private func fryingPanPlace(sender: UIPanGestureRecognizer) {
        ...
        if sender.state == .ended {
            ...
            ManagerThen.shared.windownsPostion = CGPoint(x: recordX, y: recordY)
        }
    }
