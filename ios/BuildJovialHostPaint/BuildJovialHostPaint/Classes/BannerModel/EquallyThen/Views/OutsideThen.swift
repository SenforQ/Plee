...
/*: "icon_randow_user_bg" :*/
fileprivate let data_golfTitle:[Character] = ["i","c","o","n","_","r","a","n","d","o","w","_","u","s"]
fileprivate let noti_kitForeId:String = "offer near golder_bg"
...
class OutsideThen: UIView {
    // MARK: - UI Components

    //: private let leftAvatarView = AvatarView()
    private let leftAvatarView = StraddleLiteralReactiveCompatible()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = StraddleLiteralReactiveCompatible()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = StraddleLiteralReactiveCompatible()
    ...
    private func dormer() {
        ...
        centerBgView.image = UIImage.adjust(name: (String(data_golfTitle) + String(noti_kitForeId.suffix(5))))
    }

    //: private func updateAvatarDisplay() {
    private func yankSecond() {
...
extension OutsideThen {
    // MARK: - Public Methods

    //: func configure(avatars: [String]) {
    func noAnimaLine(avatars: [String]) {
...
class StraddleLiteralReactiveCompatible: UIView {
    //: var imageurl: String? {
    var imageurl: String? {
        //: didSet {
        didSet {
    ...
    private func noneUser() {
        //: imageView.setUrlImage(urlStr: self.imageurl ?? "")
        imageView.noneAdvancedFinish(urlStr: self.imageurl ?? "")
    }
}
