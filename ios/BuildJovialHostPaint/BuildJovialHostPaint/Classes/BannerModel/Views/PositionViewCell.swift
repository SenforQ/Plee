...
fileprivate let mainBehaviorStr:[UInt8] = [0x7d,0x7a,0x7d,0x60,0x3c,0x77,0x7b,0x70,0x71,0x66,0x2e,0x3d,0x34,0x7c,0x75,0x67,0x34,0x7a,0x7b,0x60,0x34,0x76,0x71,0x71,0x7a,0x34,0x7d,0x79,0x64,0x78,0x71,0x79,0x71,0x7a,0x60,0x71,0x70]

/*: "icon_rank_coin" :*/
fileprivate let user_depthName:[UInt8] = [0x8c,0x86,0x8a,0x8b,0xba,0x97,0x84,0x8b,0x8e,0xba,0x86,0x8a,0x8c,0x8b]

/*: "icon_lounge" :*/
fileprivate let mainFillTitle:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_rank_coin2" :*/
fileprivate let noti_findName:String = "icon_rdit chin"
fileprivate let dataMixPath:[Character] = ["o","i","n","2"]
...
class PositionViewCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.sinceConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: mainBehaviorStr.map{$0^20}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.resumeName()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .shrinkSize(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = PostureTalkingButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.adjust(name: String(bytes: user_depthName.map{$0^229}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .shrinkSize(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.thumbNumbererest(colors: UIColor.ratingConfirm(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.adjust(name: (String(mainFillTitle)))
    }
}
...
extension PositionViewCell {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func oldMaster(model: LimitationTransformable, index: Int) {
        ...
        headPic.noneAdvancedFinish(urlStr: model.headPic)
    ...
    func topDown(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: user_depthName.map{$0^229}, encoding: .utf8)! : (String(noti_findName.prefix(6)) + "ank_c" + String(dataMixPath))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.adjust(name: imageStr), for: .normal)
    }
}
